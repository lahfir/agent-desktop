...
- **Approach:** One probe family, A17:
  ...
  7. Ambiguity census: how many `AMBIGUOUS_TARGET`s the shipped exact rule would produce across the fixtures and the dogfood targets' trees, **including a count of zero-extent and duplicate bounds** (offscreen and virtualized elements collapse to shared degenerate rectangles — the promotion-flooding shape) and of ref-able elements whose only stored corroborant would be a degenerate hash (the unresolvable-by-design class U3 states). Branch: informative; the counts price U3's positive-area gate and let U7 attribute the rate.
  8. Path and geometry survival on the Electron target across app relaunch and in-page content mutation, run against a repo-controlled scratch vault with marker-named notes through the dogfood harness's Obsidian launch plumbing (`run-dogfood.ps1` owns the target lifecycle; this probe reuses it rather than growing its own) — the repo-controlled-content rule applied at probe time, so ref captures and mutation measurements derive from markers, never personal content — A7-2's survival arm ran only the native stacks, so the fallback's primary signal has never met its primary consumer. Branches: path survives → the tier order stands; path churns under mutation → geometry promotion is documented as Electron's leading tier and KTD4's wording follows the measurement; target absent or the in-page mutation cannot be staged (the lever is an edit inside a probe-created synthetic note, captured counts-only) → recorded per row, the tier order stands as designed, and U7's judged Electron `STALE_REF` rate becomes the measurement of record.
...
- **Approach:**
  1. Replace the `bool` comparator with the KTD3 composition: role gate, then `identity_match` for the id tier, then — only when `has_stable_text_identity(entry)` holds — `stable_text_match` corroboration, combined per KTD3's stated rule (incomplete keyed on the live field's `LocatorField::Unknown`, not the verdict) — never `identity_match` alone, which would silently drop the A7-3 defense.
...
- **Approach:**
  ...
  3. Geometry promotion, macOS's predicate plus one Windows-measured gate: entry has `bounds_hash` **from a positive-area rectangle** and no meaningful text identity → an `Unknown` text verdict promotes on a live bounds-hash match. A zero-extent stored hash never promotes — offscreen and virtualized elements collapse to shared degenerate rectangles, which are structurally non-unique, and a structurally shared hash must not become identity. Ambiguity discipline unchanged — only a unique geometry match resolves. Stated consequence: a no-text-identity entry whose stored hash comes from a zero-extent rectangle is **unresolvable by design** — the path may land but the candidate cannot be verified, and the settled `STALE_REF` is the intended outcome, pinned; U1 item 7's census counts this ref class so U7's Electron rate judgement attributes it rather than misreading it as fallback weakness.
...
- **Approach:**
  ...
  2. Resolution errors stamp retryability by including `retryable` (with `complete`) in the `with_details` payload — the **single write** that both sets `AdapterError`'s typed channel (the surface the loop reads) and provides the diagnostic mirror; a later `with_details` enrichment replaces details wholesale and must carry the key forward. **The split governs two different loops, and conflating them breaks `find`:** settled-for-the-adapter-loop is not query-level non-retryable — the final completed-search `STALE_REF` mirrors macOS's `resolution_complete_absence` shape exactly (`complete: true`, `retryable` derived from the underlying cause's default retryability, never a blanket `false`), because core's hydration retry and wait-selector's transient classification read the same typed channel, and a NonRetryable stamp there would fail `find` permanently on first tree churn where a single fresh re-observation recovers; `resolve_element_strict` gains the deadline loop: retry only retryable-incomplete, bounded sleep, final expiry stamps `deadline_elapsed` onto the last incomplete error.
...
- **Test scenarios:**
  ...
  - A retryable descent failure with a dead process token settles `STALE_REF` immediately (call-count pin: no second attempt); the same failure with a live token retries. The pins do not assume this corroboration branch is the only route to the dead-process settle — on build 17763 a corpse's descent reads can succeed empty (A14-9) and reach the same settled `STALE_REF` through plain non-match, which is equally correct.
  - A fake-driven redaction pin: an incomplete cause error carrying marker text in a details slot passes through the deadline stamp and the dead-token conversion, and the resulting error's message and `platform_detail` carry no marker — only structured details fields propagate.
  ...
  - `UIA_E_ELEMENTNOTAVAILABLE` on the resolved target, or a completed search that finds nothing, settles `STALE_REF`; at root resolution it remains `WINDOW_NOT_FOUND` (the A14-5 split, carried).
...
- **Files:** `crates/windows/src/tree/live_read.rs` (new) and tests, `crates/windows/src/adapter.rs` (five overrides), `crates/windows/src/tree/element.rs` and `surfaces.rs` (the KTD7 handle-payload change and its downcast consumers).
- **Approach:**
...
- **Approach:** The anchor resolver is the path-only variant of `resolve_element_strict` — macOS's shape: the anchor's path is exact from the just-walked tree, so no broad-search fallback; the landed candidate is verified by the U2 matcher; failures classify per U4, with the anchor variant's classification stated explicitly because every step is descent along a stored path that churn makes permanently wrong: a path step that lands nowhere, on a role-refuted candidate, or on a vanished node **settles the attempt immediately** — the adapter's loop absorbs only the unresponsive/transport class macOS's `retry_incomplete_until` actually retries (`resolve.rs:275-277`) — so a churned anchor fails fast and core's single fresh re-observation, the mechanism that actually generates a correct new path, runs with the remaining deadline rather than being starved by replays of the stale one. Core's hydration then re-observes from the returned handle and enforces evidence completeness itself — Windows adds no hydration logic.
...
- **Test scenarios:**
  ...
  - An anchor whose path target vanished between walk and hydration settles without consuming the deadline (call-count pin: one attempt), never resolves a neighbor, and its `STALE_REF` still satisfies core's hydration retryable-error predicate so the fresh re-observation fires.
...
| Fallback never guesses | geometry promotion fires only with a positive-area bounds_hash present and text identity absent; a zero-extent stored hash never promotes; unique-match-only; each pinned both ways | U3 |
...
- **The graded fallback's honest failure rate on Electron is unknown until U7.** U1 item 8 measures path/geometry survival across Electron relaunch and in-page mutation before U3 relies on it; what only U7 establishes is the aggregate real-world `STALE_REF` rate inside live web content. That is the designed-honest outcome — the risk is expectation, not correctness, and the dogfood reports the rate rather than hiding it.
