...
- R3. **`snapshot --surface <kind>` resolves a shell surface with no `--app`.** `crates/core/src/snapshot.rs:87`'s `resolve_window_for_surface` sends every non-`Window` surface through `list_windows`, which derives the owning application from a window inventory. Shell surfaces have no application a caller can name and — per R6 — are not in that inventory at all, so the existing path returns `WINDOW_NOT_FOUND` for every one of them.
...
- R8. **A tray item's ref resolves by its `AutomationId`, not by geometry.** The GUID identity means these refs survive the resolution path's strict re-identification without the fallback R20 covers, which is what makes clicking one safe rather than positional.
- R9. **A tray item's context menu is reachable through the existing input and surface tiers.** A right-click at the item's bounds raises a menu that the `menu` surface (R4) addresses; no `open-tray-menu` command is introduced for it.
...
- R15. **The notification path is Action Center traversal, and the listener path's unavailability is reported from a live read rather than asserted as a constant.** Measured on this host: `UserNotificationListener.Current` activates, `GetAccessStatus()` returns `Denied`, and the machine's `userNotificationListener` consent store reads `Deny`. The *cause* is not settled by that evidence, so nothing shipped claims one — the adapter reports the live status with its reason attached, and the skill documents the observed behaviour rather than a mechanism.
- R16. **`wait --notification` works on Windows**, over the same listing path, within the caller's deadline, without reopening the surface on every poll.
- R17. **Every command in this sub-phase that raises or mutates shell chrome enforces the same focus-steal floor.** That is `open-system-surface` and the four notification commands. Taking an `InteractionLease` does not supply the check — `acquire_interaction_lease` performs no such refusal — so each path tests `allow_focus_steal` explicitly and refuses with `POLICY_DENIED` **before** the surface is raised.
...
- **KTD1. Shell surfaces are reached through the UI Automation root element's children, matched by native window handle.** Measured on the UIA3 COM stack with the Action Center open and frontmost: the desktop root's children went from 8 to 9, and the ninth was the Action Center's `Windows.UI.Core.CoreWindow`, identified by `CurrentNativeWindowHandle` rather than by its localized title. The Win32 enumeration does **not** reach it: `EnumWindows` did not yield that handle in the same walk that did yield `Shell_TrayWnd`, which is the positive control that makes the negative meaningful rather than an artifact of a broken walk. **Rejected: extending `passes_filter`.** `crates/windows/src/system/window_ops.rs:16` filters on visibility, size, cloak and tool bits, and none of them is what excludes these windows — the enumeration never yields them to be filtered. **Rejected: `FindWindowW` by class and title.** Measured returning 0 for the open Action Center, and it would depend on a localized title on a host running `InstalledUICulture` es-ES. **Rejected: `GetForegroundWindow` after raising the surface.** It works, but it can only see a surface this process just raised, so `resolve_shell_surface` (KTD5) could never find an already-open surface and `snapshot --surface` would require an `open` first even when the surface was already up. **What this costs:** one root-children enumeration per resolution instead of a direct lookup, which A26-12 measures rather than assumes.

- **KTD2. The notification path is Action Center UI Automation traversal, and `UserNotificationListener` is not attempted on the hot path.** Measured shape, rooted at the `Windows.UI.Core.CoreWindow` titled "Action center": a `ListView` at `AutomationId` `MainListView`; one `ListViewHeaderItem` per source application carrying a `Title` `TextBlock` and a `DismissButton`; one `ListViewItem` per notification carrying `Invoke`, with `Title`, `Content` and `Attribution` `TextBlock` children at stable `AutomationId`s, plus `DismissButton`, `ExpandButton` and `VerbButton` children each carrying `Invoke`; and a top-level `ClearAllButton`. Every element this sub-phase needs is addressed by `AutomationId`, not by localized name. The listener path measured `GetAccessStatus() == Denied` with the machine's `userNotificationListener` consent store reading `Deny`. **The cause of that denial is deliberately not claimed.** Two explanations fit the evidence — no package identity to grant the capability against, or a machine-wide privacy consent a user could flip — and `RequestAccessAsync` was not called, so nothing shipped asserts a mechanism. What is decided is the behaviour: the listener is not consulted per call, because a shipped binary cannot depend on a per-machine consent state, and A26-3 records both readings so a later host can re-decide on evidence. **Rejected: attempting the listener first and falling back.** A fallback that fires on every call is not a fallback, it is a slower primary. **What this costs:** package-identity notification listening does not ship in Phase 2, and R15 documents the observed behaviour rather than a cause — which is weaker prose and honest evidence.
...
- **KTD11. The zero-identity fallback's real blocker is that the positive-area fact is not persisted, and that is what changes.** The first pass got this wrong and its test would have passed before the change. `provisional_geometry_candidate` (`crates/windows/src/tree/resolve_search.rs:171`) gates on `entry.geometry.bounds` — the **rect** — not on `bounds_hash`; `crates/core/src/ref_alloc_tests.rs:274` proves a snapshot with `include_bounds: false` stores `bounds: None` alongside `bounds_hash: Some(..)`. So in the A24-11 case the gate is already false and `entry_is_unverifiable` fires before any search runs. Meanwhile, where that gate *is* true, `admit_node`'s existing `Incomplete if geometry_matches(..) => Collect` arm and `classify_search`'s `1 => Resolved(0)` already admit a single zero-identity candidate, and the role comparison is already `admit_node`'s first check — so "extend the acceptance side" would have shipped nothing. **The change:** add `bounds_positive: bool` to core's `RefGeometry`, set at allocation from the live rect regardless of `include_bounds`, and have both adapters' `provisional_geometry_candidate` read it instead of the optional rect. **Rejected: dropping the positive-area condition.** A17-7 measured offscreen and virtualized elements collapsing to shared zero-extent bounds that are structurally non-unique; removing the condition would admit those. **Rejected: making `bounds_hash` `None` for zero-area rects instead.** It is fewer bytes but it silently changes `should_stop_collecting`'s ambiguity rule, which reads `bounds_hash.is_none()`. **What this costs:** a core type change that both adapters consume, so macOS gains the same fix and must be covered on the macOS lane — stated in the Definition of Done rather than discovered at review.
...
| Dismiss invoked, that entry still present on re-read | `ACTION_FAILED` | delivered unverified, retry safe | R11 / KTD6; never reported as success |
...
- The notification-area items' GUID `AutomationId`s are stable for the lifetime of the icon's registration, which is what R8 relies on. A26-11 records them across a re-read within one session; stability across an owner restart is not claimed and nothing depends on it.
---
...
- **Approach:**
  ...
  2. **Extend the redaction gate before writing a capture** (KTD14). In `capture-redaction-cli.psm1`, add `app_name`, `body` and `attribution` to `Get-CaptureCliRedactionViolationsFromText`'s field pattern and add an `actions[]` array walk modelled on the existing `path[]` handling via `Find-CliJsonArrayEnd`, requiring `Test-CliRedactionValueReduced` on each. Author one MUST-CATCH fixture **per serialized field name of `NotificationInfo`** — `app_name`, `title`, `body`, `actions[]` — so the fixture set is keyed to the shipped envelope rather than to whatever the extension happened to cover.
  3. `probe.ps1` records the **reach** row A26-1 as a paired positive and negative: with a shell surface open, whether `EnumWindows` yields the surface's handle **and** whether it yields `Shell_TrayWnd` in the same walk (the control), plus the UIA root's child count and whether the surface's handle appears among the children's `CurrentNativeWindowHandle` values, open and closed. Membership, not totals — an unchanged total proves nothing, because A26-13 records that the window survives dismissal cloaked. A26-13 itself is the open/closed predicate: `WS_EX_TOOLWINDOW`, `DWMWA_CLOAKED` and `GetParent`, both states.
  4. `02-notifications.ps1` records the **Action Center shape** row A26-2 as counts and identifiers only: presence of the `MainListView` `AutomationId`, the number of `ListViewHeaderItem` groups and `ListViewItem` entries, and for one representative entry the *set of `AutomationId`s* on its children with each one's pattern set, plus the presence of `ClearAllButton`. **No `Name` from any notification element is written.** A26-3 records the listener readings: whether `UserNotificationListener.Current` activates, the `GetAccessStatus()` value, **and the machine's `userNotificationListener` consent-store value**, so KTD2's deliberate non-claim about cause is visible in the evidence rather than only in prose.
  5. `03-tray.ps1` records the notification-area rows on the COM stack: A26-4 is the child count of the promoted and overflow `ToolbarWindow32` **beside the managed-client count for the same window**, which is the row that documents the client-stack divergence rather than a platform absence; A26-5 is which overflow window class is present, corroborating C-5 first-party; A26-11 is, per child, the control type, whether an `AutomationId` is present and non-empty, the pattern-availability set, and whether bounds are positive-area — **counts, flags and control types only, never a name or an `AutomationId` value**, since the ids are machine-local and the names are the vendor inventory KTD14 exists to keep out. A26-11 also re-reads once within the session and records whether each id is unchanged, which is the stability R8 relies on.
  6. `04-frame-identity.ps1` records A26-6: with a UWP host frontmost, the foreground window's class, the class of each child of the frame, and which child's owning process differs from the frame's — processes classified as `frame_host`/`hosted_app`/`other` rather than named. A26-7 records the Start-menu surface: which shell host owns the surface the accelerator raises and the `AutomationId` set at its root, measured because on this build the accelerator raised a search surface rather than a tile surface.
  7. `06-cost.ps1` records A26-12 with the corpus methodology — one discarded warm-up, seven timed runs, min with median and max beside it (A15-13, applied in A18-7) — for the raw platform operations: raising a shell surface, one root-children resolution, one Action Center tree read, one tray enumeration. **A26-12 is labelled a pre-implementation platform-cost reference, not the shipped command's cost**; U16 takes the product baseline through the release binary once U3 and U9 exist.
...
- **Goal:** Evaluate both open arms of the two-source detector against real hosts, record the outcome, and — for the WinUI arm's second branch — write the still-open question into §2.15 rather than declare it closed.
...
- **Approach.** **Every correction is keyed to a verbatim opening phrase, never a line number** — this unit's own first edit changes §2.14's length. Corrections are made in place with a row citation and no changelog annotation. Before editing, grep each anchor phrase and confirm it occurs exactly once.
  1. The P2-O18 command-table rows beginning *"`list-tray-items` / `click-tray-item` / `open-tray-menu`"*. Correct to record that measurement moved the notification area into the table's own generic rule: its items are `Button` elements with stable `AutomationId`s and `InvokePattern`, so they carry refs and the table's stated preference applies. Cite A26-11. Keep the overflow-class sentence and cite A26-5 beside C-5, now measured first-party.
  2. The bullet beginning *"**Open menu:** after clicking a tray item, detect the resulting popup menu via UIA focus-changed events"*. Correct to the shipped path: a right-click at the item's bounds raises a popup addressed by the `menu` surface. Cite A26-11 and KTD13's advertised-set change.
  ...
  4. The bullets beginning *"**Primary list path:** `UserNotificationListener`"* and *"**Fallback list path:** open Action Center"*. Invert the ordering and correct the `PERM_DENIED` sentence, which describes a runtime denial no longer reached. **State the measured readings, not a cause** (KTD2). Cite A26-3 and A26-2.
  5. The bullet beginning *"**`focused_window`'s frame-vs-`CoreWindow` identity"*. Correct the claim that the measurement needs a WinUI/UWP-carrying host: A10-7 measured the absence of a WinUI3/MSIX population, and this question concerns the UWP `CoreWindow` shape this box presents. Cite A26-6 and A1-3.
  6. The §2.12 residual beginning *"§2.12 made a third, vault-configured staging attempt"* and §2.14's bullet beginning *"**The Chromium/Electron arm"*. Correct the host-population claim — the search that found none did not include `cursor` — and record which KTD10 branch fired. Cite A26-9.
  7. The `A23-4` closure claim that no modern-shell host is presented by any session. Narrow it to WinUI3/MSIX. Cite A26-6 and A26-10.
  8. The P2-O18 table row beginning *"| `open-system-surface --surface <kind>` | Opens an OS shell surface"* — the phrase `open-system-surface --surface <kind>` alone occurs twice, so the row is anchored on its full opening cell. Record the measured build dependency: on this build the quick actions are a pane inside the Action Center and `quick-settings` resolves to `PLATFORM_NOT_SUPPORTED` naming `action-center`. Cite A26-2 and C-10.
...
- **Test scenarios:**
  - `check-phases-ledger-citations.ps1` passes: every `A26-<n>` cited exists, and every row whose closure names 2.14 carries a disposition.
...
- **Approach:**
  1. A kind table, one row per Windows kind, carrying how the surface is raised, the window class it presents, and whether the kind exists on the running build. `taskbar`, `system-tray` and `system-tray-overflow` are `Shell_TrayWnd`-rooted and always present, so they resolve without being raised; `start-menu` and `action-center` are raised by the shell's own accelerator; `quick-settings` is a build-conditional row resolving to a refusal here.
  2. `resolve(kind) -> Option<WindowInfo>`: enumerate the UIA root element's children and match the kind's expected class, taking the identity from `CurrentNativeWindowHandle` (KTD1). This is the read half, and it is what U5 calls for an already-open surface.
  ...
  5. **`start-menu` resolves to whatever surface the accelerator actually raises**, which on this build is a search-hosted `CoreWindow` rather than a tile surface (A26-7). That satisfies R1 — the identity of the window the surface actually presents — and the divergence is recorded in the row and documented by U14. The kind refuses only if no surface takes the foreground at all.
  6. Closing is the inverse, used by tests and by U9's session teardown: send the dismiss accelerator, poll until the surface is no longer resolvable or its cloak attribute is non-zero (A26-13 records that the window survives dismissal cloaked, so "still open" reads the cloak, not handle validity).
...
- **Test scenarios:**
  ...
  - The resolved surface's handle is absent from `enumerate_top_level` while the UIA root does yield it — KTD1 as a behavioural test, with both halves asserted so a change in either is caught.
  - On a non-Windows target the module compiles and every entry point returns `not_supported`.
...
- **Approach:**
  ...
  4. Extend Windows's `supported_surfaces()` with the shell kinds it can resolve plus `Menu`, and extend `surface_root`'s match with the corresponding arms. **The two edits are one commit** (KTD13). The `Menu` arm resolves through the same per-pid detector `menu_is_open` composes, returning the menu surface when one is open and `WINDOW_NOT_FOUND` when none is — the shape the `Sheet` arm already uses.
  5. `quick-settings` is advertised only if the build exposes it; on this build it is not, so `require_supported` refuses in core before U3's build-conditional refusal is reached. Both refusals exist and are not redundant: core's says "this adapter does not offer it", U3's says "this build does not have it", and a build that gains the surface flips the first without touching the second.
...
- **Test scenarios:**
  ...
  - **Advertised equals resolvable**: a test enumerates `supported_surfaces()` and asserts `surface_root` returns something other than `not_supported("surface")` for every entry — invert-verified by adding a variant without an arm.
...
- **Approach:**
  ...
  2. The core command calls `surface_scope::require_supported` before the adapter, so an adapter that does not advertise a kind refuses with the existing shape rather than each adapter inventing one.
  ...
  5. Registration walk, each point with a test or a compile error behind it: `commands/mod.rs`; the `lib.rs` re-export; the `Commands` variant with its `#[command(about = …)]`; the `dispatch()` arm; `command_policy`'s exhaustive `match cmd`, a **compile** failure if missed; the batch string-to-variant arm; `contract_tests.rs`'s `coverage_names()` and `ADAPTER_PASSTHROUGH_COMMANDS`. The command-count literal in `contract_tests.rs` moves by one — **read the current literal at implementation time rather than trusting a number written here**.
...
- **Dependencies:** U3.
- **Files:** `crates/windows/src/notifications/{mod,session,read,list,actions,verify,listener_status}.rs` (new) and their `_tests.rs` siblings, `crates/windows/src/lib.rs`, `crates/windows/src/system/adapter.rs`.
- **Approach:**
  1. `session.rs` owns opening and closing the Action Center around a batch of work, built on U3. It records whether the surface was already open on entry and restores that state on exit, and teardown runs regardless of the wrapped result so a session is never leaked on an error path. It exposes an explicit lifecycle — acquire and release — because U10's wait holds one across many reads; the restore-on-exit contract binds to that lifecycle, not to each `list_notifications` call.
  ...
  8. `listener_status.rs` **reads the live access status and reports it with the reason attached**, rather than returning a constant. A constant would make its invert-verification test the constant instead of the platform (KTD2).
...
- **Test scenarios:**
  ...
  - `listener_status` reports the live status with a reason — invert-verified by stubbing the read to `Allowed` and watching the reported value change, which a constant could not do.
...
### U10. `wait --notification`, the session lifecycle and the policy floor

- **Goal:** Make `wait --notification` work on Windows without reopening the surface every poll, and make the focus-steal floor one rule both the direct command and the wait inherit.
- **Requirements:** R16, R17.
- **Dependencies:** U9.
- **Files:** `crates/core/src/adapter/system.rs`, `crates/core/src/commands/wait.rs`, `crates/windows/src/notifications/session.rs`, `crates/windows/src/notifications/wait_tests.rs` (new), `crates/windows/src/system/adapter.rs`.
- **Approach:**
  1. `wait --notification` is not a separate command: `crates/core/src/commands/wait.rs`'s `wait_for_notification` already polls and diffs a `NotificationFingerprint` multiset. Windows inherits it once `list_notifications` works.
  2. **The wait needs an explicit session hook, because the core loop has none.** Without one, the only way a Windows session could span polls is hidden adapter state that keeps the Action Center open across calls — which contradicts U9's restore-on-exit contract and makes the session-open count unobservable. Add a bracketing pair to the adapter trait — `begin_notification_watch` / `end_notification_watch`, defaulting to `Ok(())` — that the core wait calls around its loop. macOS keeps the defaults and its behaviour is unchanged; Windows acquires and releases U9's session lifecycle, and the count of session opens becomes a value a test can assert.
  3. The floor is the adapter's (U9 step 7). This unit's only policy work is ensuring the wait inherits it: the first `list_notifications` inside the loop refuses, so a strict-headless wait fails at policy rather than after its full deadline.
  4. macOS is unaffected: it keeps the trait defaults, and its notification listing already opens Notification Center.
- **Non-goals for this unit:** event-subscription-based waiting. `docs/phases.md` names it "where supported"; the listener path that would supply it is not consulted (R15), so the poll is the path and saying so is the decision.
- **Patterns to follow:** `crates/core/src/commands/wait.rs:263-365` for the fingerprint-diff loop, reused unchanged apart from the bracketing calls.
- **Test scenarios:**
  - `wait --notification` returns a notification posted during the wait and not one already present at its start — the baseline half a naive implementation gets wrong.
  - The wait opens the Action Center **once** across a multi-interval wait — asserted on the session-open counter step 2 makes observable, invert-verified by moving acquisition inside the loop.
  - The surface is left in its entry state when the wait times out.
  - A strict-headless `wait --notification` refuses at policy on the first poll, not after the deadline.
  - An adapter using the trait defaults behaves exactly as today, proven by macOS's wait tests being unchanged.
- **Verification:** an agent can block until a notification arrives without the machine being unusable for the duration, the session's span is an observable value rather than an implementation detail, and macOS's behaviour is untouched.
...
### U13. Zero-identity: persist the positive-area fact in core

- **Goal:** Make the A24-11 content shape resolvable by fixing the actual blocker — that the positive-area fact is lost when a snapshot hides bounds — rather than extending a gate that is already false in that case.
- **Requirements:** R20.
- **Dependencies:** U1.
- **Files:** `crates/core/src/ref_geometry.rs`, `crates/core/src/ref_alloc.rs`, `crates/core/src/ref_alloc_tests.rs`, `crates/windows/src/tree/resolve_search.rs`, `crates/windows/src/tree/resolve_search_tests.rs`, `crates/macos/src/tree/resolve_search.rs`, `crates/macos/src/tree/resolve_search_tests.rs`.
- **Approach:**
  1. Add `bounds_positive: bool` to `RefGeometry` (`crates/core/src/ref_geometry.rs`), `#[serde(default)]` so an older refmap deserializes, set at allocation from the live rect **regardless of `include_bounds`**.
  2. Change both adapters' `provisional_geometry_candidate` to read `entry.geometry.bounds_positive` instead of `entry.geometry.bounds.is_some_and(..)`. Everything downstream — `geometry_matches`, `admit_node`'s existing single-candidate admission, `classify_search`'s `1 => Resolved(0)`, `should_stop_collecting`'s ambiguity rule and `admit_node`'s role check — is already correct and is not touched. The bug was never in the acceptance side.
  3. **This is a core change both adapters consume, so macOS gains the same fix** and its lane must cover it. That blast radius is the cost KTD11 names; it is taken here rather than at §2.15 because R20 is one of §2.14's own exit criteria and cannot be closed without it.
  4. **`crates/windows/src/tree/resolve_search.rs` is at 384 of 400 lines.** If the change does not fit, split the geometry-candidate logic into `resolve_geometry.rs` rather than compressing it — the cap is a structural rule, not a budget.
- **Non-goals for this unit:** changing `has_meaningful_identity`, which is core-owned and shared; widening what counts as identity would change macOS behaviour beyond this fix.
- **Patterns to follow:** the existing `geometry_matches` and `admit_node`/`candidate_outcome` arbitration, read before editing so the change stays confined to the eligibility predicate.
- **Test scenarios:**
  - **A ref allocated with `include_bounds: false` from a positive-area nameless element resolves.** This test **fails before the change** — which the first version of this unit did not, and is why the unit was rewritten. Confirm it fails first, then make it pass.
  - `allocate_refs_keeps_bounds_hash_when_snapshot_hides_bounds` is extended to assert `bounds_positive` is `true` for the positive-area node and `false` for a zero-area one — the allocation half, without which the resolution half could pass on a constant.
  - A zero-area element is never admitted, preserving A17-7's exclusion — invert-verified by forcing `bounds_positive` true for a zero-extent node and watching a structurally non-unique element get admitted.
  ...
  - **The same three cases pass on macOS**, run on the macOS lane, since the predicate change is shared.
  - An older refmap without the field deserializes with `bounds_positive: false` and behaves as it does today — the serde-default path, which is what keeps a mid-session upgrade from panicking.
- **Verification:** the content A24-11 measured at a 75% stale rate resolves and acts; every path that could resolve to the *wrong* element still refuses; the allocation and resolution halves are each pinned; and the change is proven on both adapters rather than only the one that motivated it.
...
- **Approach:**
  ...
  4. Document the behaviours an agent would otherwise discover the hard way:
     ...
     - `wait --notification` holds the Action Center open for the duration of the wait rather than reopening it per poll — a difference from macOS (R16).
     ...
     - `start-menu` resolves to the surface the shell's accelerator actually raises, which on pre-Windows-11 builds is search-hosted (A26-7).
...
- **Goal:** Prove the shell and notification surfaces work against the real desktop through the release binary, with every effect confirmed by an observation the binary did not perform.
- **Requirements:** R1, R7, R10, R11.
- **Dependencies:** U4, U9.
- **Files:** `tests/e2e-windows/scenarios/ShellSurfaces.ps1` (new), `tests/e2e-windows/scenarios/Notifications.ps1` (new), `tests/e2e-windows/NotificationStage.psm1` (new), `tests/e2e-windows/Run-E2E.ps1`.
- **Approach:**
  1. Two new scenario files, each **explicitly registered**: a dot-source line and a sequence entry in `Run-E2E.ps1`. The harness does not auto-discover scenarios, so an unregistered file is a silently skipped test.
  2. `ShellSurfaces.ps1`: open the Action Center, assert the returned identity snapshots and carries the expected root landmark, find a ref inside it, assert the surface closes. Then snapshot `--surface system-tray` and assert the ref count matches a count the harness takes itself through the COM shim — `Assert-Effect`'s contract is that the read side is independent of the command under test, so a count read through the same binary would not qualify.
  3. `Notifications.ps1`: stage a notification the harness owns, list it, dismiss it, assert by re-reading that it is gone.
  4. **The staging mechanism is named, not asserted.** `NotificationStage.psm1` registers a Start Menu shortcut carrying a harness-owned AUMID during stage setup, posts through `ToastNotificationManager.CreateToastNotifier(<aumid>)`, and removes the shortcut at teardown — the documented route for an unpackaged process to post a toast that lands in the Action Center. A **precondition leg** asserts the posted toast is present before the dismiss leg runs; if it is not, the scenario fails rather than skipping. The AUMID is harness-generated and contains no machine-local path, so it does not widen the redaction surface.
  5. Both scenarios obey the contract rules: targets via `Find-Target` (rule04), no direct envelope field access outside `Lib.psm1`/`LibEnvelope.psm1` (rule05), `Register-Legs` and skip tokens up front (rule08), every desktop-touching leg inside `Enter-Stage` (rule09), the binary only through `Invoke-GuardedAgent` (rule10), status reads via `--property` (rule12), files under 400 lines (rule13), no automatic-variable assignment (rule14).
  6. Shell surfaces take the foreground, so both scenarios acquire the desktop lease and foreground stage in the documented lock order (`DesktopLease → ForegroundStage → MenuStage`). Opening the Action Center outside `Enter-Stage` would race every other scenario.
  7. **`Lib.psm1` is at 400 lines.** Any new helper goes in `NotificationStage.psm1` or another new module, never into `Lib.psm1`.
- **Non-goals for this unit:** running the live suite in CI. `windows-e2e.yml` has no registered runner and §2.15 owns registration; these run locally and their results are reported in U16, the same position §2.12's scenarios are in.
- **Patterns to follow:** `tests/e2e-windows/scenarios/Surfaces.ps1` for an existing surface scenario; `Lib.psm1`'s `Assert-Effect`/`Assert-NoEffect` for the read-act-reread contract; `LibVerdict.psm1`'s leg ledger.
- **Test scenarios:**
  - `scripts/check-e2e-windows-contract.ps1 -SelfTest` passes over the new files, including rule17's file-set equality with `git ls-files`.
  ...
  - `Run-E2E.ps1 -SelfTestSeedFailure` still exits non-zero, proving the new sequence entries did not break failure propagation.
  ...
  - The notification scenario fails when the dismiss is skipped, rather than passing because the toast expired — invert-verified, and the difference between testing a dismiss and testing a timeout.
  - The precondition leg fails the scenario when staging does not produce a visible notification, so a machine where the AUMID route does not work reports a failure rather than a pass.
  - Both scenario files and the new module are under 400 lines.
- **Verification:** the shipped surfaces are driven through the real binary against the real desktop, every effect is confirmed by a read the binary did not perform, the notification leg controls its own fixture rather than depending on the machine's state, and the suite's structural gate accepts the new files.
...
- **Approach:**
  ...
  2. **Take the product cost baseline here, through the release binary**, with the corpus methodology — one discarded warm-up, seven timed runs, min reported with median and max beside it (A15-13, applied in A18-7). A26-12 is a pre-implementation platform reference and is not the shipped path's cost; this is. `scripts/perf-baseline-compare.sh` is structurally macOS-bound and is not the vehicle.
...
| R6 | the surface's handle is absent from `enumerate_top_level` and present among the UIA root's children, both asserted | U1, U3 |
| R7 | `snapshot --surface system-tray` yields refs whose count matches the harness's own independent COM read | U5, U15 |
| R8 | a tray item's ref carries a non-empty `native_id` and resolves without the geometry path | U1, U15 |
| R9 | a right-click at a tray item's bounds raises a surface the `menu` kind resolves | U5, U15 |
...
| R15 | `listener_status` reports the live status; stubbing the read to `Allowed` changes the reported value | U9, U14 |
| R16 | `wait --notification` returns only a notification posted during the wait, and the session-open counter reads 1 across a multi-interval wait | U10 |
...
8. R16's session hoisting — move acquisition inside the poll loop and watch the session-open counter assertion fail.
9. R15's live read — replace `listener_status` with a constant and watch the stubbed-`Allowed` test stop changing.
...
11. R20's zero-area guard — force `bounds_positive` true for a zero-extent node and watch a non-unique element get admitted.
...
4. The notification area is listable and clickable **through refs and the generic command surface**, promoted and overflow both, with no Windows-specific tray command shipped — and `docs/phases.md`'s P2-O18 table corrected to record why.
5. The four notification commands work against the Action Center, matching the macOS JSON contract field by field, with every mutation verified against the entries it targeted, an unrecognised tree reported rather than returned as empty, and `wait --notification` working over one held session.
...
10. A ref from nameless positive-area content resolves under the persisted positive-area fact, with the ambiguity and zero-area guards each invert-verified, **and the shared core change proven on the macOS lane as well as the Windows one**.
...
| Action Center notification module (session, read, list, actions, verify, listener status) and tests | ~1,700 | yes |
| `wait --notification` session bracketing and the adapter-side floor | ~250 | yes |
| Frame identity and `--app` descent | ~350 | yes |
| Zero-identity: core `RefGeometry` field, allocation, both adapters' predicate, tests on both lanes | ~250 | yes |
| Menu-detector third source, if an arm measures one as needed | ~200 | yes |
| E2E scenarios, the notification staging module, harness helpers | ~800 | yes |
| **Product code total** | **~5,000** | **yes** |
| `skills/agent-desktop-windows/`, `skills/agent-desktop/`, `README.md`, `docs/phases.md` | ~500 | documentation |
| Probe area 26 (six scripts) plus captures, rows and redaction fixtures | ~900 | evidence, exempt |

**`docs/phases.md` sanctions an overage only when the plan says so explicitly — this paragraph is that statement.** Product code is forecast at roughly **2.5x** the origin's `~2k`, taken deliberately (session-settled: user-directed, chosen over splitting into §2.14 and §2.14.1).

The forecast fell by roughly 800 lines between the first and second versions of this plan, entirely because re-measuring on the product's own client stack removed a subsystem: KTD4's three tray commands, their three registration walks, a `TrayItem` core type, an MSAA module and a manifest feature all disappeared, and what replaced them was one sentence in `docs/phases.md`. That is the most useful number in this table — the second-largest line item was work the first version would have built for a capability the platform already provided.
...
- **Re-measurement changed three decisions, and the same trap can recur.** The first version of this plan took its UIA readings on the managed client stack and reached two wrong conclusions, one of which would have written a false correction into the source of truth. KTD3 is the standing rule that prevents it; A26-4 records both stacks' readings side by side so the divergence is visible in the ledger rather than only in this plan's history.
...
- **The command count literal in `contract_tests.rs` moves by one.** **Mitigation:** U4 instructs the implementer to read the current literal rather than trusting this plan's number.
