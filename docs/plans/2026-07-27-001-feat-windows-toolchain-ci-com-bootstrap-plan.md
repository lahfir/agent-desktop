...
- ~~Does the seam cover `write_user_atomic` / `write_user_file`~~ (`crates/core/src/refs.rs:208`)? **Decided: no — the exclusion is deliberate.** Routing user output through the seam would be a regression: its two callers (`commands/screenshot.rs:22`, `commands/clipboard_get.rs:51`) write to a destination the *user* named, which must be allowed to sit on a network share, behind a OneDrive/mapped/junction path, or in a directory owned by another principal — exactly what the locality, reparse and `TokenOwner` gates refuse for private artifacts. Unix already encodes the asymmetry (`private_file_tests.rs` — the user path accepts the group-readable destination and `/tmp` that the private path rejects), the two policies were always distinct (`ensure_private_portable`/`validate_private_destination` vs `ensure_user`/`validate_user_destination`/`sync_user_directory`), and U7's temp-handle reorder is shared by both regardless. Documented on `write_user_atomic` and pinned by `user_output_writes_deliberately_bypass_the_platform_seam`. The accepted cost: user output keeps the portable rename-based replace on Windows, which unlike `ReplaceFileW` fails over a destination another process holds open.
