...
**KTD4 — The Windows overlay is a detached child of the same binary, reached over a named pipe.** *(session-settled: user-approved — chosen over an in-process render thread: the CLI is stateless per invocation, so the renderer must outlive the process that started it.)* The child is guarded exactly as macOS guards its own — it spawns only when `std::env::current_exe()`'s file stem is `agent-desktop`, so an FFI host, whose `current_exe()` is the host process, never forks one. The first control reaches the child over its inherited stdin, and later controls over the pipe, matching the macOS bootstrap so a connect race against a pipe that does not exist yet cannot occur.
  ...
  **The stdin bootstrap starts the child; it never answers `rendered`.** stdin is one-way and the child's stdout and stderr are null, so the first Enable has no return path down the channel that carried it. The child therefore creates its window **first** and its pipe server **second**, and writes the Enable acknowledgement to its first connected client. The parent, having written the bootstrap, polls `CreateFileW` on the pipe name within the enable budget and reads that byte. Pipe connectability then *means* the window exists, which is what makes it a usable readiness signal rather than the weaker "a process started" that a bare spawn would report.
  **Only a `Present` control may bring a renderer into existence.** After a failed connect, a `Disable`, `Hide` or `Show` returns without spawning, mirroring `crates/macos/src/system/cursor_overlay/spawn.rs`, which refuses on `is_disable() || (is_transient() && !is_travel())`. Without that refusal `cursor-overlay disable` would spawn a renderer in order to tell it to stop, and - worse - `src/dispatch/mod.rs` sends `Hide` before and `Show` after **every** mutating command in a headed overlay-enabled session, so a headed session would fork a detached renderer per command.
  **A failed connect is classified before it is acted on.** `ERROR_FILE_NOT_FOUND` means no renderer and spawns. `ERROR_PIPE_BUSY` means a live renderer that has not yet re-armed `ConnectNamedPipe`, so the caller waits on `WaitNamedPipeW` within the control's budget and retries; treating it as absence would fork a duplicate, drop the control it carried, and then answer `rendered: false` while an overlay is drawing.
...
2. The predicate is an **exhaustive `match` over the `Role` enum** in `crates/core/src/roles.rs`, beside `is_mutable_value_role` and clearly distinguished from it in its doc comment, so a role added later fails to compile until someone decides which side it belongs on.
...
5. **The ledger gate has no vocabulary for step 3 and gains one.** `13-ledger-check.ps1` requires every `DEFERRED` row to name a closure inside `2.0`–`2.16`, so re-pointing `A21-2` out of Phase 2 would fail the very gate this unit must pass. Add a `closure: post-phase-2` token accepted alongside the range, with its own MUST-CATCH and MUST-PASS fixtures — the gate is code and needs its own test, which is why the token does not ship without them.
...
3. `spawn.rs` tries to connect first, classifies a failure before acting on it, and spawns only for a `Present` control (KTD4): not-found spawns, busy waits and retries, and a `Disable`, `Hide` or `Show` returns `Ok(())` without spawning. Spawning is additionally guarded on `current_exe()`'s file stem being `agent-desktop`; that guard returns `Err`, not `Ok`, so a refusal cannot become `rendered: true` (KTD15). The child is spawned `DETACHED_PROCESS | CREATE_NO_WINDOW` with the env marker carrying the protocol generation and the pipe name, the first control written to its stdin, and stdout/stderr null so the child can never contaminate the JSON envelope.
3a. After a spawn, the parent polls `CreateFileW` on the pipe name within the enable budget and reads the child's Enable acknowledgement — the bootstrap's stdin is one-way and cannot answer `rendered` (KTD4).
...
4. `child.rs` is entered from `run_cursor_overlay_child` before clap parsing, rejects a marker whose generation is not current, reads its bootstrap control from stdin, **creates and shows its window first**, then creates the pipe with `FILE_FLAG_FIRST_PIPE_INSTANCE | PIPE_REJECT_REMOTE_CLIENTS` (KTD12) and acknowledges the Enable to its first client — so pipe connectability means the window exists. A child that loses the first-instance race exits before creating anything. On each connection it resolves the peer with `GetNamedPipeClientProcessId` and compares `TokenUser` SIDs before reading the payload (KTD13). The control loop matches `CursorOverlayControl` exhaustively, so `Show` and `Hide` cannot be silently unhandled.
...
8. **Measure, do not assume, that the child survives its parent.** A parent inside a job object with `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` takes a detached child down with it. Measure it on the dogfood host in both directions — inside such a job and outside one — and record the reading as a ledger row before any doc claims the child outlives the invocation.
...
- Two protocol generations resolve to different pipe names, so a stale child is unreachable rather than subtly wrong.
...
4. **The `rendered` tests need a seam, because they cannot run under their own gate without one.** `cargo test -p agent-desktop --lib` runs from a test binary whose `current_exe()` stem is not `agent-desktop`, so the spawn guard refuses — the same refusal U12 asserts as correct. The spawn entry point therefore takes an injectable executable path, and the tests drive that. Without it the two central scenarios here would silently exercise the guard instead of the feature.
...
- **Ending a session out-of-band** — the manifest marked ended without a `disable` ever being sent — leaves no child and no overlay pixel within one idle tick.
