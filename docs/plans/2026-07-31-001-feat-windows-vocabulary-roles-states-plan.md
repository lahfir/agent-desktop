...
- **Approach:** In-place corrections, never annotations, each citing what disproved it:
  ...
  7. **The PR-size cap (`:799`).** It reads "at or under 2,000 changed lines (excluding `Cargo.lock`, the generated FFI header, and vendored fixtures)", and every Windows sub-phase has blown through it — 2.0 at 21,048 insertions, 2.1 at 4,921 against a ~1.3k estimate, 2.2 at 8,244. A rule violated by every instance including two already merged is not being enforced as written; it is being applied to something narrower that nobody wrote down. Amend the exclusion list to name what practice already excludes — probe corpora, committed captures, and dogfood reports — so the cap governs hand-written product code, which is what it was for. The residual overrun of this sub-phase's own Rust against that corrected cap stays visible in the Goal Capsule rather than being defined away.
  8. **Unobserved ref-able control types.** Any `ControlType` mapping to an `INTERACTIVE_ROLES` member that U9 could not exercise gets a named receiving sub-phase in `docs/phases.md`, with the reason it could not be observed on this box. A ref-able role whose arm has never run is a gap the next planner must be able to see without reading a dogfood report.
  9. **`CONCEPTS.md`.** Add entries for role, state vocabulary, name evidence, and `native_id`, in the style of the existing entries. They were single-platform code types until this sub-phase; they are now shared vocabulary two adapters produce.
...
- **Who owns `subrole`, `role_description`, `placeholder`, and `dom_classes`?** `docs/phases.md:867` permits Phase 2 to add them and no sub-phase through Phase 2 or Phase 3 claims them; 2.3 completes only `native_id`, and an agent receives none of the four on any platform until someone does. The sources are known — `role_description` ← `LocalizedControlType`, `placeholder` ← `HelpText`, `dom_classes` ← WebView2's `HtmlClass` — so this is an ownership question, not a technical one. **Recommendation: 2.4.** It already does Chromium detection and the web-wrapper work where `dom_classes` lives, and it is the sub-phase that first assembles a full snapshot node. 2.3 does not assign it, because scoping another sub-phase is its planner's call and not this one's.
