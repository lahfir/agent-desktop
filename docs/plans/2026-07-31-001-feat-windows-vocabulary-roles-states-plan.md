...
- **KTD2. `ControlType` is the key; `ClassName` and pattern availability are the refinement.** Governs R2, R3. A2-4 measured the counterexample and named this sub-phase in its action clause. Three refinement inputs, in this order of preference, all already available or added by U2:
  ...
  3. **`AriaRole`** (`UIProperty::AriaRole`, 30101) — the direct Windows analogue of macOS's subrole fold, and the property Microsoft's own ARIA table is written against. Populated by web and XAML content, empty on plain Win32; usable where present.
...
- **KTD6. Three of core's state tokens have no macOS producer, and UIA can produce them.** Governs R5. `crates/core/src/state.rs:17-31` reserves `invalid`, `multiselectable` and `haspopup` explicitly *"for adapters/platforms that can emit it"*. UIA has direct analogues: `IsDataValidForForm` (`UIElement::is_data_valid_for_form`), `SelectionPattern::can_select_multiple`, and — for `haspopup` — `IsExpandCollapsePatternAvailable` on a non-container role or the MSAA `STATE_SYSTEM_HASPOPUP` bit. 2.3 is the first sub-phase that can make these tokens real. It should, where U1 shows the source is readable; where it does not, the token stays unproduced and that is recorded, not faked. This is the opposite of scope creep: leaving them unproduced when the platform supplies them is what would need justifying.
...
- **Approach:** Extend the standing probe workflow 2.2 landed rather than adding a second one — it already triggers on `pull_request`, so it runs on this PR without touching `main` or `ci.yml`. **Three edits are required, not one**, because the workflow hardcodes the 2.2 probe at every step: widen the `paths` filter to include `probes/windows/15-vocabulary/**`; add a run step invoking the new probe under `shell: powershell` (Windows PowerShell 5.1 — `pwsh` is used nowhere in `probes/windows`); and extend the upload step's `path` to cover the new captures. Note `if-no-files-found: error` — a probe step that produces no capture fails the job rather than passing quietly, which is the behaviour to keep. Widening only the filter yields a job that triggers and re-runs 2.2's probe, which looks green and measures nothing.
  ...
  7. **`AriaRole`** — never read. Is it populated on the fixture stacks, and is it empty rather than erroring on plain Win32? KTD2 uses it only where present.
  8. **The RawView-versus-ControlView delta** — node count and distinct `ControlType` set for the same root under both walkers, per target. KTD11 needs the number, not an argument.
  **Pre-committed actions.** If `LabeledBy` cannot be read or resolved, `labelled_by_text` stays `None` and U6 records the slot as unproduced on Windows rather than approximating it from `Name`. If `LegacyIAccessible.DefaultAction` is uninformative, KTD4's `LegacyIAccessible` arm contributes no action and the gap is recorded for 2.7. If a pattern-state property cannot be distinguished from a genuine value when the pattern is absent, U5 gates that state on the corresponding `Is*PatternAvailable` property and classifies the ungated case `Absent`, never a default-looking token. If `AriaRole` is unreadable, refinement uses pattern availability and `ClassName` alone. "Unmeasurable" is a branch, never a silent revert to inference.
...
- **Approach:**
  ...
  3. Extend `WALK_SET`. `cache.rs` iterates it and needs **no edit**; this is the whole reason 2.2 built it that way, and it is why KTD5's decision to read pattern state as properties costs no new cache machinery at all.
...
- **Approach:** A base `match` from `uiautomation::types::ControlType` onto `agent_desktop_core::Role` with **no catch-all arm** — 41 variants, totality by compilation (KTD1). Read `ControlType` through 2.2's property path as an integer rather than through `UIElement::get_control_type()`: that accessor is `ControlType::try_from(i32)` over an enum with no fallback (`uiautomation` 0.25.0 `core.rs:585-591`), so an unrecognised or vendor id returns `Err` and conflates "a control type I do not know" with "the read failed". The integer path preserves 2.2's tri-state: an unmapped integer is `Role::Unknown` with the integer recorded in the error shape, a failed read is `LocatorField::Unknown`.
  Then refine, per KTD2, in preference order: pattern availability, `ClassName`, and `LocalizedControlType` only if U1 cleared it. Resolve explicitly, not by discovery: the `Tab`/`TabItem` inversion; `Edit`/`Document` → `textfield` (A2-4); `Button` + Toggle → `switch` or `checkbox`; `Button` + ExpandCollapse → `menubutton`; `ListItem` + SelectionItem → `option`; `Custom` and `Pane`, which carry no inherent semantics and should map to the honest container role rather than a guess.
...
- **Test scenarios:**
  ...
  - Every arm Microsoft's published `ControlType`↔ARIA-role table covers agrees with that table, resolved through core's own `roles::normalize_role_query` where ARIA and canonical names differ (`spinbutton`→`incrementor`, `gridcell`→`cell`, `textbox`→`textfield`). The expectation set is transcribed from **Microsoft's table**, not from the map, which is what makes this the one correctness assertion KTD1's proof (a) actually executes rather than narrates. A deliberate divergence from the table is allowed but must be listed explicitly, so it reads as a decision rather than a bug.
...
- **Approach:** Three steps, in this order.
  ...
  **The `LabeledBy` read crosses an element boundary and therefore escapes the per-element secure gate** (KTD10). Read the target's own `IsPassword` in the same fetch as its `Name`, and withhold the derived `labelled_by_text` when it is true. Adding `labelled_by_text` to `VALUE_BEARING` alone does not close this — that protects the referring element, not the element the text came from.
...
- **Approach:** This unit is not a test run; it is an inspection with fixes. Unit tests can only assert what the implementer already believed. A role map is exactly the artifact whose errors are invisible to its author and obvious in a real tree.
  **Targets — one per UI stack, because the stacks disagree** (A2-4, A7-1). Run U8's tool against each, on the developer machine:
  1. classic Notepad — Win32, served by the client-side `EDIT` proxy
  2. an Explorer folder window — DirectUI shell
  ...
  5. a Chromium or Electron application if one is present — settling the tree first, since A1-5 measured a first read understating it about 13x
  6. Settings, if the box presents it — A10-7 records that this Server 2019 box carries no WinUI3 or MSIX population
...
- **Goal:** `docs/phases.md` stops describing infrastructure that does not exist and scope that has moved, and `CONCEPTS.md` gains the terms this sub-phase makes cross-platform.
- **Requirements:** R12.
- **Dependencies:** U1, U3, U4, U5, U6, U7, U9.
- **Files:** `docs/phases.md`, `CONCEPTS.md`.
- **Approach:** In-place corrections, never annotations, each citing what disproved it:
  1. **§2.3 conformance-test claim (`:1013`).** "must pass the same conformance tests U1/U2 already run for macOS, parameterized over `WindowsAdapter`" describes a harness that does not exist. What U1/U2 left is core-owned, adapter-agnostic vocabulary predicates (`Role::is_canonical`, `roles::is_canonical_role`, `state::assert_states_in_vocabulary`) plus a per-platform test *pattern* proven once by macOS at `crates/macos/src/tree/roles.rs:150-216`. Rewrite to describe what Windows actually must do.
  2. **§2.3 exit criterion (`:1021`).** "vocabulary conformance tests span every UIA `ControlType` (complete mapping coverage, not a sample)" invites the tautological table this repository has already been burned by. Rewrite as KTD1 states it: totality by exhaustive `match` over the crate's control-type enum, correctness by canonicity, the `INTERACTIVE_ROLES` producer obligation, live fixture assertions, and the real-application run.
  3. **§2.3 scope (`:1011-1015`).** Add available actions from pattern availability, citing A2-1's action clause and 2.2's `walk_available_actions` seam. Correct the `AutomationId` → `native_id` bullet to record that the mechanism shipped in 2.2 and 2.3 verifies it (KTD9). Record that the `states` slot required plumbing rather than a seam (KTD3).
  4. **§2.3 estimate (`:1023`).** ~1.5k LOC predates the states plumbing and the accname reconciliation; replace with the measured figure this PR lands.
  5. **The accname contract (`:772` and §2.3's `:1015`).** "Core accessible-name computation takes precedence over adapter-supplied `NameEvidence`" was true as intent and false as a call graph. Record what U6 changed so the statement is true.
  6. **Core invariant 2 (`:819`) and §2.4 (`:1031`, `:1041`).** All three specify `ControlViewWalker`; `crates/windows/src/tree/walker_source.rs:31` opens the raw-view walker. Correct them to state what shipped, why the map is valid over the superset, and where ControlView genuinely applies — citing U1's measured delta.
  7. **Unobserved ref-able control types.** Any `ControlType` mapping to an `INTERACTIVE_ROLES` member that U9 could not exercise gets a named receiving sub-phase in `docs/phases.md`, with the reason it could not be observed on this box. A ref-able role whose arm has never run is a gap the next planner must be able to see without reading a dogfood report.
  8. **`CONCEPTS.md`.** Add entries for role, state vocabulary, name evidence, and `native_id`, in the style of the existing entries. They were single-platform code types until this sub-phase; they are now shared vocabulary two adapters produce.
...
| Role correctness | every emitted role is canonical; every arm Microsoft's published `ControlType`↔ARIA table covers agrees with it, with any deliberate divergence listed; every `INTERACTIVE_ROLES` member this adapter claims has a producer and every unproduced role is listed; the `Tab`/`TabItem` inversion is pinned; **no test asserts an equality table transcribed from the map's own arms** | U3 |
...
| **Dogfood: it was run** | U8's tool was run against every U9 target on a developer machine; each target's stack is named; every absent target is recorded as **skipped with a reason**, never green | U9 |
...
- **Does this sub-phase land as one PR, given it exceeds the 2,000-changed-line cap?** `docs/phases.md:799` caps a sub-phase PR at 2,000 changed lines excluding `Cargo.lock`, the generated FFI header, and vendored fixtures; this plan budgets ≈2.2–2.6k of hand-written Rust before evidence artifacts. Every Windows sub-phase so far has exceeded it — 2.0 at 21,048 insertions, 2.1 at 4,921 against a ~1.3k estimate, 2.2 at 8,244 — so the cap is either being applied to something narrower than "changed lines" or is not being enforced, and neither reading is written down. **The sub-phase does not split cleanly**: U3–U5 produce vocabulary that nothing reads until U7 fills the seams, so a partial 2.3 ships dead code, and U9 is what establishes the vocabulary is correct at all. If the owner wants the cap honoured, the one genuinely separable piece is **U6** — the accname reconciliation is the only unit touching `crates/core` and `crates/macos`, it carries its own guard in the macOS golden fixtures, and Windows can supply `native_title` alone until it lands. Recommendation: land as one PR and record the overrun, or split U6 out first; do not split U3–U5 from U7.
- **Should `offscreen` mean the same thing on both platforms?** macOS computes it geometrically; UIA reports it, and A14-8 measured a UIA value that contradicts itself within one window. KTD7 chooses the provider's per-element value for Windows, which is the honest Windows answer and is not the macOS answer. Whether the cross-platform contract should specify one is a question for 2.15's integration review, not for this sub-phase.
- **Where do `subrole`, `role_description`, `placeholder`, and `dom_classes` land?** `docs/phases.md:867` permits Phase 2 to add them and no sub-phase through Phase 2 or Phase 3 claims them; 2.3 completes only `native_id`. `LocalizedControlType` is the natural `role_description` source and 2.3 deliberately leaves it unread (see Deferred to Follow-Up Work), so whichever sub-phase claims the field pays for the property read then. The question is who claims it, not what feeds it — and an agent receives none of these four fields on any platform until someone does.
- **Does the repo-wide "Definition of Done: Performance Baseline" apply to Windows sub-phases?** `scripts/perf-baseline-compare.sh` is a macOS/AX harness. U5's pattern-derived state reads are the first thing in the Windows adapter with a real per-node cost, and there is no in-repo calibration path for them. This wants a one-line ruling for every remaining Windows sub-phase, and it was already raised by 2.2 without an answer.
- **At what node count does `CacheRequest` stop being a pessimization?** Carried forward unresolved from 2.2. It gains weight here: KTD5 adds pattern caching on top of property caching, and the crossover for patterns has never been measured at all.
- **Should `LocatorEvidence.value` be produced by this sub-phase?** It is populated today from `ValuePattern.Value` by 2.2's read set, but no sub-phase scope claims it, and whether a control's value is vocabulary or live state is genuinely ambiguous. 2.3 leaves the existing behaviour alone and flags it rather than silently adopting it.
