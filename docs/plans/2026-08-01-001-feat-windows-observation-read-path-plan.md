...
- **Execution profile:** One PR from `feat/windows-2.4-observation` into `feat/windows-adapter`, never `main`. Budget ≈2k lines of hand-written Rust per the origin estimate; probes, captures, and the dogfood report are evidence artifacts outside the cap per the cap's own exclusion list. The budget is a target, not a promise — U2's core schema work and five net-new system inventories were not priced separately by the origin line; if the product figure exceeds the cap, it is stated in the PR the way 2.3's was, with the clean split seam named (U2 is the only unit touching core and can land as its own PR if the owner wants a split; U4–U5 are separable from U6–U8 only at the cost of shipping enumeration nothing snapshots through). Conventional Commits.
...
- **2.4 is planned as `docs/phases.md` defines it, with contradictions corrected rather than planned around.** (session-settled: user-directed — the instruction was to plan 2.4 from the source of truth; research found five §2.4 statements that are false today, and each is corrected by U10 with its disproving evidence rather than silently obeyed or silently dropped.) Governs R13. See KTD1, KTD5, U10.
...
- KTD3. **Window identity is HWND-first, corroborated by a process-generation token and the window's own record, failing closed.** `WindowInfo.id` is `"w-{hwnd}"`; `process_instance` is a token derived from the owning process's creation time, mirroring macOS's `"macos-proc-v1:{start_seconds}:{start_microseconds}"` shape (`crates/macos/src/system/process_identity.rs:81-86`). Every resolution re-verifies pid, token, app, **and title** before trusting the id — the title check is what the cited macOS `verify_window_record` carries as its per-window signal (`crates/macos/src/system/window_resolve.rs:144-167`), and it is load-bearing here too: the process token alone cannot catch an HWND destroyed and reused by another window of the **same still-running process**, which would otherwise re-verify cleanly and resolve to the wrong window — the A7-3 silent-wrong-target class. Inventory assembly re-verifies identity on both sides of the read, per `window_inventory.rs:91-155`. A recycled HWND on a different process generation returns `WINDOW_NOT_FOUND`. No probe has measured HWND recycling directly (U1 measures what it can); the fail-closed shape is what makes the **cross-process** unmeasured case safe, and the title corroboration is what covers the intra-process one.
...
- **Approach:** One probe family, A16, with a pre-committed branch per question:
  ...
  11. Connection-to-settled latency on the Chromium target, min-of-seven with a discarded warm-up (A15-13). A1-5 measured a 13-of-172-node first contact but never the settle time, and core retries only inside the request deadline. Branch: settle exceeding the default operation deadline → the still-thin `platform_detail` guidance also carries a deadline recommendation, and U9 judges Obsidian with that number in hand.
...
- **Approach:** Per KTD4: a new flattened optional group (the 7-field cap on `NodeIdentity`/`NodePresentation` forces it), `#[serde(skip_serializing_if = "Option::is_none")]` per field (`Vec` variant for `dom_classes` with the empty-vec skip), threaded through evidence projection the way existing identity fields are. macOS supplies none of the four in this sub-phase — its `AXSubrole` reads continue to refine `role` only. Core's `MockAdapter` tests gain nodes carrying each field to pin serialization both ways.
- **Patterns to follow:** `crates/core/src/node_identity.rs` / `node_presentation.rs` (flattened optional groups with serde skips); `crates/core/src/live_locator/observed_tree.rs:174-175` (projection point).
- **Test scenarios:**
...
- **Test scenarios:**
  ...
  - An id whose stored title no longer matches the live window — the same-process HWND-reuse shape — fails with `WINDOW_NOT_FOUND`, and the test fails if the title corroboration is removed (KTD3).
...
- **Files:** `crates/windows/src/tree/{root_resolve,observe}.rs` and tests; `crates/windows/src/adapter.rs` (`ObservationOps` override); `crates/windows/src/tree/walker.rs` (boundary-count seam only).
...
- **Approach:** Per KTD6/KTD7. Detection from root window class/provenance; shell-shape from U1's census; activation = settle (the connection already triggered the build); the error only from full-depth walks; `platform_detail` names Chromium's `--force-renderer-accessibility` switch when still thin, plus the U1-derived deadline recommendation where item 11's branch fired. The Windows lease override lands here because the loop acquires it before activating — without it the first settle dies `ACTION_NOT_SUPPORTED`. The wrapper predicate consumes only evidence already read — control type, name, value, `AutomationId`, actions — so the skip costs nothing per node. Surfaces: root resolution for `Window`/`Focused`, and the modal-as-sheet check that tests the focused window **itself** before its children (`WindowIsModal` is already in the read set), mirroring macOS's focused-window-is-surface pattern.
...
- **Test scenarios:**
  ...
  - A wrapper with a name, an `AutomationId`, or an action consumes depth (three separate pins).
  ...
  - The still-thin case carries `platform_detail` naming the flag, and the message contains no target-derived text.
...
| Error redaction | failed reads against marker-named controls produce errors whose message, details, and `platform_detail` carry no marker | U3–U8 |
...
- `docs/phases.md` reads true — the five known corrections plus whatever U1/U9 disproved, each with cited evidence — and `CONCEPTS.md` carries the concepts 2.4 introduced.
...
- **The ~2k LOC estimate predates U2's core schema and five net-new inventories.** If the product figure exceeds the cap, the PR states it and names U2 as the clean split seam, exactly as 2.3 handled its overrun.
