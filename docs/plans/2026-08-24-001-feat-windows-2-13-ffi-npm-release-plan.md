...
- **Approach:**
  ...
  5. Add the dry-run seam, and understand the hazard it exists to remove before writing it. **A `workflow_dispatch` run of `release.yml` today publishes for real.** The manual step emits `release_created=true` unconditionally after validating the version and tag (`release.yml:60-82`), and `publish_npm` / `publish_skills` both default to `true` (`:19, :24`) — so dispatching the workflow to "just build the artifacts" would create a GitHub Release and push to npm from a sub-phase branch. The seam is a `dry_run` boolean input defaulting to `false`, threaded through `release-please`'s outputs, with `publish-github` gaining `if: needs.release-please.outputs.dry_run != 'true'` — it has no condition of its own today, and the two publish jobs skip behind it through `needs`. A normal release run is unaffected because the default preserves current behaviour; a dry run builds and uploads artifacts and stops. U9 dispatches with `dry_run: true`, and states in its record that it did.
