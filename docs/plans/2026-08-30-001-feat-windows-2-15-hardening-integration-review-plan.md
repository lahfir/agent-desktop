...
Planning re-measured every load-bearing claim in that scope against the current branch rather than planning from the document. Twenty-one need correcting, and ten of those are outright false about the code. Two rig items the document called blocked are measurable on this box today. Two "what closes it" prescriptions describe fixes to code that already does the thing. One defect is one line, not the resolver rewrite the document describes. The corrections are the first implementation unit, because every later unit and every decision below reads from the corrected text.
...
| R14 | An `--app`-scoped wait resolves its target on terms that fit the event class: an appearance-class wait honours its timeout rather than refusing `APP_NOT_FOUND` before the loop starts, and a disappearance-class wait treats an unresolvable target as its answer rather than an error. |
| R15 | `app-terminated` is not reported for a process that is provably still alive. |
| R16 | `offscreen` carries one documented meaning across adapters, with the per-adapter fidelity difference and its mechanism stated rather than left to be discovered. |
...
| R20 | A self-hosted interactive Windows runner is registered against `[self-hosted, Windows, agent-desktop-e2e]`, its trigger policy re-ratified against what the workflow actually declares, **fork-PR approval enabled repository-wide**, workspace hygiene between runs verified by observation, its ephemeral-versus-persistent choice recorded, and the accumulated queue flushed. |
...
| R36 | A Windows launch that resolves a window reports its `renderer`, so core's unprompted `--cdp` guidance fires against an Electron target there as it does on macOS. |
...
These are closed decisions, not open promises. Each names the blocker that makes closing it impossible on the hosts this phase has, and U1 writes each into `docs/phases.md`.

- **macOS keeps the trait default for `spotlight`, `dock` and `menu-bar-extras`**, per the disposition §2.15 authorizes. KTD17 names the blocker: an interactive macOS host, which has never existed for this line.
- **Multi-monitor `list_displays` verification, mixed-DPI bounds, per-display capture, RDP/locked/Session-0 degradation, and live WGC pixel success** are out of reach within Phase 2 on the hosts available to it. KTD16 states each reason separately.
...
One item, and it is a genuine deferral rather than a ratification — the work remains owed and a receiving phase is named for it.
...
- **KTD9. `app-terminated` is confirmed against process liveness, and the confirmation fails open.** Windows's `list_apps` population is the distinct owning processes of admitted top-level windows (`crates/windows/src/system/app_ops.rs:142-186`), and `capture_signal_baseline` matches it deliberately so the two agree about what an app is. A close-to-tray process hides its last visible window, fails `passes_filter`'s `visible` check, leaves the population while fully alive, and `diff_apps` — which keys purely on `(pid, process_instance)` set membership — reports a genuine `app-terminated`. **The fix is one adapter call at the one place it matters:** before `wait_event` reports an `AppTerminated`, it reads `SystemOps::process_state` for that identity, which is already on the trait with a `not_supported` default and both adapters overriding it, and `wait_event.rs:16` already holds `&dyn PlatformAdapter`. **The confirmation suppresses the event only when the process is proved alive.** An error, a `not_supported` default, or any ambiguous read emits the event exactly as today — a liveness check that fails closed would silently eat real terminations, which is worse than the bug it fixes. **This is the only arm of this gate that can remove an answer, and it cannot fire on macOS:** macOS derives its app population from `NSRunningApplication`, so membership already implies the process is alive and the confirmation always passes. The suppression exists for Windows, where the window-derived population is what makes the false positive possible (KTD24). **Rejected: changing the apps population to the process table.** `app-launched` would then fire for every background service start. **Rejected: ratifying and documenting that `app-terminated` means "stopped owning a window".** The event's name is cross-platform and would then be true on neither adapter's terms. **What this costs:** one process-state read per reported termination, on macOS too, where it always passes.

- **KTD10. `offscreen` gets one documented meaning and two documented fidelities, ratified — because both attempts to make it one predicate were worse than the divergence.** *Draft one* had Windows emit geometry **or** the provider's `IsOffscreen` while macOS stayed geometric: that is not a normalization at all, since the same virtualized row still answers differently on the two adapters. *Draft two* promoted macOS's predicate into core and had Windows drop `IsOffscreen`. Measurement killed it on two grounds. **(a) Windows has no window rectangle to compute against.** `resolve_states` (`crates/windows/src/tree/states.rs:84-90`) receives no rectangles, `window_bounds` does not exist anywhere in `crates/windows`, and `walk_from_root` enters at an arbitrary element with no window-rect concept — macOS threads it down through `BuildContext.window_bounds`. The shared predicate would have returned `None` where `IsOffscreen` returned a value: a silent capability regression, in a unit whose file list did not even name the modules that would have to change. **(b) `IsOffscreen` has four consumers, not one.** Beyond the canonical token it feeds `ElementState.offscreen` (`crates/windows/src/tree/live_read.rs:207-210`), which is the **first arm of the actionability visibility gate** (`crates/core/src/actionability/gates.rs:20-26`), plus `scroll_ladder.rs` and `scroll_into_view.rs`, and it is one of six `READ_HEALTH_PROBES`. Dropping it would have made a below-fold target stop failing the visibility gate. **And the evidence was overread:** A14-8's action clause says derive `offscreen` per element and never propagate a container's value to its subtree — not discard the signal — while A18-1 measured a WPF list item flipping `IsOffscreen` over a 40px scroll, which is scroll-viewport clipping that no window-disjointness test can reproduce. Windows genuinely knows more here than macOS can compute. **What ships is the contract, stated so one sentence is true of both adapters:** `offscreen` means *no part of the element is presently rendered within the visible region of its container*. The "no part of" is load-bearing and not stylistic: without it a half-overlapping element reads either way, while macOS's predicate is full disjointness and UIA's `IsOffscreen` is likewise entirely-clipped — and this sentence lands in `docs/phases.md` as settled contract text, where an ambiguity becomes the next planner's wrong decision. macOS approximates that geometrically against the window rectangle, because it is the only source it has — no macOS element publishes `AXOffscreen` (`crates/macos/src/tree/state_reader.rs:86-88`). Windows reads the provider's per-element value, which additionally sees scroll-viewport clipping. **One question, two fidelities, both honest, and the difference documented rather than discovered.** A14-8's no-propagation ban is restated as binding on both. **Rejected: normalizing to geometry.** It costs a design change Windows has no inputs for and breaks three subsystems. **Rejected: normalizing to the provider value.** macOS has none to read. **What this costs:** an agent that branches on `offscreen` gets a more precise answer on Windows than on macOS, which R16 now states rather than hides — and the two drafts that tried to hide it were each worse than saying so.
...
- **KTD12. macOS adopts Windows's superset matching for dangerous shortcuts — the one macOS GA behaviour change this gate ships.** Windows matches a blocked combo when the key matches and the pressed modifiers are a superset of the entry's (`crates/windows/src/input/blocked_combo.rs:34-43`), because `alt+shift+tab` steals the foreground exactly as `alt+tab` does. macOS compares canonical strings for equality and covers the same ground by enumerating variants — it lists `cmd+q` and `cmd+shift+q` separately. Both are honest; only one cannot miss a variant nobody listed. **The matcher changes and the entry list does not.** Under superset matching `cmd+shift+q` and `ctrl+cmd+q` do become redundant — `{cmd,shift}` and `{ctrl,cmd}` each contain `{cmd}` — and an earlier draft therefore deleted them. **They stay.** Deleting a redundant entry is only safe if the redundancy argument is right, and this plan got that exact argument backwards once already: a first draft kept `ctrl+cmd+q` on reasoning that was inverted, and it would have gone into `docs/phases.md` as settled contract text. Keeping all five entries makes the change **monotonic** — every combo blocked today stays blocked, by construction rather than by my arithmetic — and the only delta is that additional dangerous variants join them. On a platform this gate cannot run live (KTD24), a change that provably cannot unblock anything is worth more than a tidier list. **What this costs:** two list entries that are arguably redundant survive, which is the cheapest possible insurance against being wrong about why. **The evidence decides it, not a rule about which adapter is cheaper to change:** an enumeration cannot cover a variant nobody listed, and adopting the enumeration on Windows would install that hole rather than close it. **Rejected: ratifying the divergence.** "Is this combo dangerous" answered by two rules means a combo blocked on one platform and delivered on the other, which is a safety difference, not a formatting one. **What this costs:** a macOS user who previously could send `cmd+ctrl+shift+q` now gets it blocked. It ships **alone** as satellite `2.15.5b` with a release note, and it is the single most visible thing this gate changes for an existing user.
...
- **KTD13. Nine divergences are ratified rather than normalized, each because normalizing is either infeasible on Windows or would remove a capability that works.** Each is recorded in `docs/phases.md` by U1 with the reason and evidence; a ratification that lives only in this plan is "Recorded", which the DoD rejects.
  ...
  9. **`presentation` stays macOS-only, ratified.** It is derived from `NSRunningApplication`'s activation policy and Windows has no equivalent source at any point in a process's life — not a placement problem, an absent signal. **`renderer` is *not* ratified with it, and an earlier draft of this plan was wrong to bundle them.** That draft claimed both were "bundle-derived metadata Windows has no equivalent source for at launch time", which is true of `presentation` and false of `renderer`: §2.15's own text says "the detector is available rather than missing ... the open question is placement and evidence, not feasibility", and the placement resolves cleanly — `observe_window` runs at `crates/windows/src/system/launch.rs:48` and `:56`, **before** `launch_result` is built at `:86`, so for every launch that produces a window the class-based detector (`crates/windows/src/tree/chromium.rs`) can classify it at exactly the point `LaunchResult` needs the answer. **What ships:** Windows sets `renderer` whenever the launch resolved a window, which is the Electron case the unprompted `--cdp` nudge exists for. **What is ratified, on its own honest reason:** a launch that resolves no window reports no `renderer`, because there is nothing yet to classify — not because Windows lacks the signal. Writing the bundled infeasibility claim into `docs/phases.md` would have been exactly the false correction this gate exists to prevent, and it was caught by review rather than by measurement.

- **KTD14. `agent-desktop-windows` is Windows-only by design, and the exclusion is recorded rather than the 25 errors fixed.** The crate does not compile for `x86_64-unknown-linux-gnu` — 25 errors at HEAD, all unresolved items behind `#[cfg(target_os = "windows")]`, up from the 8 `docs/phases.md` records at the merge-base. `docs/phases.md` pre-authorizes "a recorded exclusion with its reason", and the reason is that **nothing consumes it**: the binary and the FFI crate take the windows crate through target-gated dependencies, `agent-desktop-macos` not compiling off macOS is already accepted in the same document (phases.md:2699), and `ci.yml`'s Windows lane already `cargo check`s the crate on its own platform. The repo learning it is measured against — never ship platform code that CI cannot execute — is about **core** carrying platform code; core passes the Linux target check today. **One thing ships with the exclusion.** `docs/phases.md:1407` offers "a lane ... **or a recorded exclusion with its reason**" and then adds "plus the stub repairs the check surfaces" — a clause that is vacuous once no check surfaces anything, so U1 rewrites it so the document does not contradict the shipped decision. **Rejected: also gating the crate consistently at the module level.** An earlier draft called that a bounded tidy-up; it is 265 non-test item-level `#[cfg(target_os = "windows")]` sites across dozens of files, it belongs to no unit, and — decisively — it would *remove the very unresolved-item errors the exclusion documents*, leaving the crate compiling empty on Linux and the recorded exclusion describing a failure that no longer happens. The half-gated stubs are inert: no non-Windows target links this crate. **Rejected: fixing the 25 errors.** Building a non-Windows facade for a crate no non-Windows target links is the over-engineering this sub-phase was asked to avoid. **Rejected: adding the lane.** A lane whose only purpose is to make a facade compile is the same work with a yellow badge on it.
...
- **KTD18a. Registration is the owner's to authorize, not the implementer's to take — and the plan stops at that boundary.** `docs/phases.md:1277` declined registration at §2.12 **on the owner's decision**, recording the reason in full: "registering a self-hosted runner on a public repository is an outward-facing security action the repository owner reserves, and GitHub's own guidance advises against self-hosted runners on public repositories at all, because any fork PR can otherwise execute untrusted code on a machine the owner controls." §2.12 routed the *work* to §2.15. **It did not transfer the *decision*, and this plan must not read it as if it had.** The security review of this very gate independently re-derived that same threat — runner labels are repository-wide and five workflows here trigger on `pull_request` (KTD18b) — which is corroboration of the owner's reasoning, not a refutation of it. Enabling fork-PR approval narrows the exposure; it does not make the call someone else's.
  **Pre-committed branches.** *Branch A — the owner authorizes registration:* U10 proceeds exactly as KTD18b specifies, and R20, R21 and R22 discharge as written. *Branch B — the owner maintains §2.12's refusal:* R20, R21 and R22 **cannot be met by this gate**, and that is recorded in `docs/phases.md` as blocked on a decision the owner reserves, restating §2.12's reasoning rather than inventing a new one, with the live-gate exit criterion carried to §2.16 — the last sub-phase, which merges to `main` — or ratified as out of reach for Phase 2. **Every other unit in this plan lands unchanged under either branch**; only U10 and U11 hang on it, so the branch does not gate the gate.
  **Rejected: registering and reporting it afterwards.** An outward-facing security action taken on someone else's behalf is not made acceptable by being correct. **Rejected: treating §2.12's routing of the work as authorization.** That is the reading that turns a reserved decision into a task, and it is the reading an implementer would take from the plan's earlier wording. **What this costs:** the largest single block of this gate's verification value waits on one answer, and the plan says so instead of assuming it.

- **KTD18b. Once authorized, the runner is persistent, launched from a Task Scheduler task at log-on, and its trigger policy is re-ratified against what the workflow actually declares.** §2.12 shipped `windows-e2e.yml`, `scripts/run-windows-e2e-ci.ps1` with its two-condition refusal guard, and `docs/runbooks/windows-self-hosted-runner.md`; the runbook's own last section is "What §2.15 still owes". A service-mode runner has no desktop and cannot see UIA, so the runner runs inside a real interactive session. **Persistent, not ephemeral/JIT:** a JIT runner re-registers per job, and the interactive session it needs is the session a human logged into — there is nothing to recreate per job, and workspace hygiene between runs is the runbook's existing procedure. **The trigger policy re-ratifies to what is declared and no more:** `workflow_dispatch` plus a paths-scoped `push` on `feat/windows-adapter`, never `pull_request`. **§2.15's instruction to set the fork-PR approval policy stands, and an earlier draft of this plan was wrong to call it moot.** That draft reasoned only about `windows-e2e.yml`'s own triggers — it has no `pull_request` trigger, so it concluded there was no fork PR for a policy to gate. **Self-hosted runner labels are repository-wide, not workflow-scoped.** `ci.yml`, `codeql.yml`, `windows-capability-probe.yml`, `release-dry-run.yml` and `supply-chain.yml` all trigger on `pull_request` today, and a fork PR that edits any one of them to add `runs-on: [self-hosted, Windows, agent-desktop-e2e]` would execute on a box with a real interactive desktop the moment that workflow runs. The runbook says exactly this — the approval setting exists to keep a *different*, less carefully scoped workflow from becoming the exposure — and the draft answered a narrower question than the runbook asked. **So registration confirms "Require approval for all outside collaborators" (or stricter) is enabled, independent of any one workflow's triggers**, and U1's correction C19 is withdrawn: §2.15 was right and the plan was wrong. **The queue is flushed at registration:** five runs exist, four cancelled and one queued as of 2026-08-30, and `cancel-in-progress: false` means the queued one would claim the runner the moment it registers, on a commit that is not the one under test. It is cancelled before registration completes.
...
- **KTD24. No change in this gate may alter an unverifiable platform's behaviour in a way only a live run could detect.** This gate exists to prove a branch is production-grade, and it reaches `agent-desktop-core` and `agent-desktop-macos` in six units. **The verification those changes actually get is CI's macOS lane — compile, `clippy -D warnings`, and unit tests. Nothing live, ever.** `native-e2e.yml` has zero runs in the repository's history (KTD17) and the Windows perf vehicle cannot measure macOS (KTD20). A gate that hardens Windows by quietly loosening the platform it cannot run has failed at its own job. So every core- or macOS-reaching change is classified before it ships, and only three classes are allowed:

  - **Envelope-additive** — adds a field or changes a message; cannot change what any adapter *does*.
  - **Strictly-additive behaviour** — can only produce a more correct answer, never suppress one that fires today.
  - **Inert by construction on the unverifiable platform** — the code runs there but the condition it keys on cannot arise, for a reason stated and tested rather than assumed.

  A change that suppresses an existing answer on a platform with no live verification is **not allowed in this gate**, however right it looks. The audit:

| Change | Reaches | Class | Why it is safe, or what was changed to make it safe |
|---|---|---|---|
| U2 `data.rendered` + trait default | core, Linux stub | envelope-additive | The field is new. The default flip makes a non-implementing adapter *honest* rather than changing what it does — Linux rendered nothing before and renders nothing now, and it now says so. `submit.rs` stays fail-soft, which is what guarantees no action on any platform can fail because of this (R4). |
| U3 resolver constructors + `stale_ref` messages | core, macOS | envelope-additive | Pure functions returning error structs, with the payload pinned to a literal by a core test. No I/O, no timing, no control flow. The ten macOS sites change message text, which no consumer branches on — agents read `code`. |
| U4 `body` / `actions` redaction | core, macOS traces | envelope-additive | Redaction only. Costs a macOS-visible diagnostic boolean in one locator error payload, stated in KTD5. |
| U6 wait-event semantics | core, macOS | strictly-additive **except one arm**, which is inert on macOS | The seen-set and both `--app` scoping arms can only produce an event that is missed today, or spend a timeout instead of refusing early — none of them suppresses. **The `app-terminated` liveness confirmation is the one suppressing arm, and it cannot fire on macOS:** macOS derives its app population from `NSRunningApplication`, so a process in that population is alive by construction and the check always passes. U6 scenario 10 pins that. It suppresses only on Windows, where the window-derived population is what makes the false positive possible. |
| U7 `.exe`-tolerant predicate | core, macOS | inert by construction, with one honest caveat | macOS names come from `localizedName` and do not carry the suffix, so the new arm never fires — **except for a Wine- or CrossOver-wrapped application genuinely named `Foo.exe`**, where `--app Foo` would newly match. That is a widening, not a suppression, and an ambiguity resolves to `AMBIGUOUS_TARGET`. Claiming it "never" fires would have been the easier sentence and the false one. |
| U8b superset blocked combos | macOS | strictly-additive — **and this is what changed to make it so** | See KTD12: the matcher changes and **the entry list does not**. |

  **Rejected: shipping the macOS changes and relying on review to catch a regression.** Review reads diffs; a wrong event semantic on a platform nobody runs is invisible to it. **Rejected: deferring every macOS-reaching change out of the gate.** Four of the six are envelope-additive and carry no behavioural risk at all; deferring them would trade zero risk for real duplication left in place.
...
- **Registration of the self-hosted runner is not a decision this plan takes at all.** It is owner-reserved per §2.12, pre-committed to two branches in KTD18a, and U10 stops and asks. It is listed here so it is not mistaken for one of the reversible calls below.
- **The user is the owner for three further decisions this plan takes on their behalf and flags rather than defers:** KTD12's macOS GA behaviour change, KTD16's **four** rig ratifications, and KTD17's ratification that macOS stays without the three shell-surface kinds. Each is stated with its reason and is reversible at review. KTD16's other two rig outcomes — the 740 measurement and the A26-13 attempt — are measurements rather than ratifications, so there is nothing there to reverse.
...
- **Requirements:** R1, R2, R16, R19, R23, R25, R26, R32, R33, R34, R35.
- **Dependencies:** none.
...
- **Approach:**
  ...
  3. **`13-ledger-check.ps1` repairs.** The closure-sub-phase range becomes `2.0-2.16` — **`$MaxClosureMinor` at `probes/windows/13-ledger-check.ps1:11` is what enforces it**, and line 142 is only the failure message; editing the message alone leaves the gate rejecting `closure: 2.16` and would make this unit's own invert-verification fail; the completeness half is retired with its reason stated in the script's own message (KTD19); `RequiredAreaIds` gains the areas that exist and are not listed, and the success message stops hardcoding "all 11 evidence areas covered" — it derives the count from `$RequiredAreaIds` instead, or this unit's scenario 1 cannot pass.
  ...
  8. **The 2.15-assigned ledger sweep.** List every `probes/windows/FINDINGS.md` row whose action column names 2.15 and dispose of each — implemented by a unit here, or re-assigned in `docs/phases.md` with its reason. Listing them is mechanical and belongs in `13-ledger-check.ps1`; judging whether each was honoured is the reviewer's obligation at close, which is exactly what the Cross-cutting DoD says. **The sweep annotates at `2.15.0` and enforces at the gate PR, because it cannot do both at once:** `2.15.0` merges first and touches `probes/**`, so the capability-probe lane runs the check on that very PR, while the dispositions it would demand — A21-2's closure among them — only exist once U9 lands in the gate PR. So at `2.15.0` each 2.15-naming row is annotated with the unit that owns it, `owned by U<n>` is a valid interim state, and the no-undisposed-row assertion is what fails at the gate. This is the check that would have caught A1-3 assigning the UWP descent to §2.4 six sub-phases before anyone noticed.
  9. **Three wording corrections the decisions above make necessary.** The "lands as its own sub-phase PR" phrase becomes "a separate reviewable PR" — **one correction applied to all nine of its occurrences** (`docs/phases.md` 1419, 1420, 1425, 1426, 1430, 1431, 1432, 1435, 1440), because correcting one leaves the document self-inconsistent — so it stops implying the Cross-cutting DoD's per-sub-phase review, dogfood and baseline obligations for a 70-line matcher change (KTD2). The Linux-target bullet's "plus the stub repairs the check surfaces" clause is rewritten, because with the exclusion recorded there is no check to surface anything and the clause would leave the document contradicting the shipped decision (KTD14). And `offscreen`'s new single predicate is written with its limit named — a virtualized row scrolled out of view but geometrically inside its window is not flagged on either adapter (KTD10).
  10. **KTD10's `offscreen` contract.** One sentence that is true of both adapters — *no part of the element is presently rendered within the visible region of its container* — with each adapter's source named beside it (macOS geometric against the window rectangle because `AXOffscreen` is not published; Windows the provider's per-element value, which also sees scroll-viewport clipping, per A18-1), and A14-8's no-propagation ban restated as binding on both.
  11. **Three more decision writes, each answering a scope bullet that would otherwise have no owner.** KTD21's shell-identity scope, with the §2.14 exit-criterion clause corrected — §2.15 quotes it as "an identity `snapshot --window` consumes" and planning could not locate that string verbatim in §2.14, so the real clause is identified before it is edited. KTD22's ratified per-poll notification cost with its measured number. KTD23's class-(b) Windows-only lifecycle envelope set, with the UIPI activation entry **deleted** (§2.12 measured the opposite) and the protected-process pair corrected to `INVALID_ARGS`/`not_delivered`.
...
- **Test scenarios:**
  ...
  3. `scripts/check-phases-ledger-citations.ps1` exits 0 — every `A<n>-<m>` id this PR cites in `docs/phases.md` resolves to a real `FINDINGS.md` row. **The CONTRADICTS-backed-by-a-hunk invariant is a different script's** (`13-ledger-check.ps1`, scenario 1); an earlier draft attributed it here, where the strings do not appear.
  ...
  6. **R2 and R16 are reviewer-verified, and the plan says so instead of inventing a check.** An earlier draft routed both through "a check in `scripts/check-e2e-windows-contract.ps1`'s doc-rules module" — that module implements PowerShell hygiene rules over `tests/e2e-windows/**` and never reads `docs/phases.md`; no script in the repository asserts anything about that file's prose. Building one would mean a keyword-presence grep over free-form text, which false-fails on any later paraphrase and false-passes on a word that appears without the coverage behind it. The reviewer at close reads §2.15's scope against its exit criteria and confirms each capability appears, exactly as the Cross-cutting DoD assigns the honoured-row judgement to the reviewer. **This is the plan's one requirement pair without a mechanical guard, and the Definition of Done names it as such** rather than letting a fake check imply otherwise.
  7. `docs/phases.md`'s `offscreen` text carries one meaning sentence, both adapters' sources, and the no-propagation ban — reviewer-verified at close, on the same terms as scenario 6.
  8. At `2.15.0`, `13-ledger-check.ps1` lists every row naming 2.15 and each carries either a disposition or an `owned by U<n>` annotation. At the gate PR the interim state is no longer accepted: a row naming 2.15 that is neither closed nor re-assigned to a different sub-phase fails the check. **Both halves are asserted** — a check that only ever ran at `2.15.0` would pass on annotations forever.
  9. The class-(b) envelope list in `docs/phases.md` carries no UIPI activation entry and names `INVALID_ARGS` for the protected-process refusal, and the same three rows appear in this plan's Error and Disposition Mapping. **The two are asserted against each other**, so a later edit to one without the other fails.
...
- **Approach:**
  1. `SystemOps::update_cursor_overlay`'s default body becomes `Err(AdapterError::not_supported("update_cursor_overlay"))`. macOS overrides and is unaffected. **Linux's behaviour does not change — only its honesty does:** it rendered no overlay before and renders none now, and `cursor-overlay enable` there stops claiming otherwise. Because `submit.rs` stays fail-soft (R4), no Linux or macOS action can fail as a result. U1 records it.
...
- **Approach:**
  ...
  5. **The misuse guard is a scoped call-site check, not a runtime assertion.** An earlier draft proposed a whitespace `debug_assert!` inside the constructor; it would have panicked two existing tests that pass sentences today (`crates/core/src/commands/pointer_action_tests.rs:119` passes `"terminal target"`, and `crates/core/src/live_locator/resolve_query_hydration_tests.rs:32` does the same), and it would have been absent from release builds anyway. **What ships instead:** those two tests are corrected alongside the production sites — the same defect, in tests — and a check counts non-test callers **per constructor**, because there are two and they are distinct. **The numbers, so the check has a baseline rather than a guess:** `AdapterError::stale_ref` has **17** non-test callers today and **2** after this unit (`crates/core/src/app_error.rs:33`'s delegation and `crates/core/src/refs_store.rs:76`); `AppError::stale_ref` has **2** and stays at 2. **The exclusion is every `#[cfg(test)]` item, not every `*_tests.rs` file** — an earlier draft said the latter, which leaks the inline `mod tests` blocks in `crates/ffi/src/commands/mod.rs` and `envelope_out.rs` into the count and would fire the day someone adds an inline unit test, failing for the wrong reason. A unit test pins the message format against a ref-id argument.
...
- **Test scenarios:**
  ...
  7. A check counts non-test callers separately for the two constructors, excluding every `#[cfg(test)]` item, and pins them at **2 and 2**. It fails if either grows. Counting them together, excluding only `*_tests.rs`, or counting tests would each make it fail for the wrong reason and then be weakened until it guarded nothing.
...
### U7. One `--app` predicate, error envelopes that name a real recovery, and `renderer` on a window-observed launch

- **Landing target:** satellite **2.15.4**.
- **Goal:** every Windows command accepts the same identifier set, a failed `--app` says what form it wanted (KTD7), and a Windows launch that saw a window says what renderer it is (KTD13.9).
- **Requirements:** R11, R12, R36.
- **Dependencies:** U1.
- **Files:** `crates/core/src/app_info.rs` (which carries its tests inline, so the predicate's new rule is tested there rather than in a new file), `crates/core/src/app_lookup.rs`, `crates/core/src/adapter_error.rs`, `crates/windows/src/system/window_ops.rs`, `crates/windows/src/system/window_ops_tests.rs`, `crates/windows/src/system/launch.rs`, `src/tests/`.
- **Approach:**
  ...
  5. `launch_result` sets `renderer` from the window it was just handed, using the shipped class detector. The window is already resolved at that point (`observe_window` at `launch.rs:48`/`:56` precedes `launch_result` at `:86`), so this reads a value in hand rather than adding a probe. A launch that resolved no window leaves the field absent, as today.
- **Test scenarios:**
  ...
  7. Launching an Electron target on Windows **without** `--cdp` returns `renderer: chromium` and core's `launch_suggestion` fires the `--cdp` nudge. **Invert-verifiable — neither is returned today.**
  8. Launching a non-Chromium target returns no `renderer` and no nudge, and a launch that resolves no window returns no `renderer` — the guard that the detector did not become a blanket.
...
### U8. `pressed`, and superset shortcut matching

- **Landing target:** **two** satellites. `pressed` is Windows-only tree-state work and lands as **2.15.5a**. The macOS blocked-combo change lands **alone** as **2.15.5b** with its release note, because it is the one change to shipped GA behaviour and it needs a reviewer looking at input safety rather than at tree state.
- **Goal:** the two divergences this gate normalizes rather than ratifies (KTD11, KTD12). `offscreen` is not here: KTD10 ratifies it, and U1 writes the contract.
- **Requirements:** R17, R18.
- **Dependencies:** U1.
- **Files:** `crates/windows/src/tree/states.rs`, `crates/windows/src/tree/states_tests.rs`, `crates/macos/src/input/blocked_combo.rs`, `crates/macos/src/input/blocked_combo_tests.rs`, `skills/agent-desktop-windows/`, `README.md`.
- **Approach:**
  1. **`pressed`.** `TreeProperty::LegacyState` is already cached and already read by `push_legacy_state`, so this adds no property and no round trip: `resolve_states` emits `pressed` for a node whose role resolved to `button` and whose already-read `LegacyIAccessible` state carries the `STATE_SYSTEM_PRESSED` bit. `button_role` is untouched — measurement found its reclassification accurate on every control that reached it, and macOS already maps `AXSwitch` to `switch`, so the role never diverged. `states.rs:53-72`'s doc comment is rewritten from "deliberately unproduced" to what produces it.
  2. **Superset shortcut matching on macOS.** `crates/macos/src/input/blocked_combo.rs` adopts Windows's rule — the key matches and the **pressed** modifiers are a superset of the entry's. **`BLOCKED` is not edited.** All five entries stay, so no combo blocked today can become unblocked no matter how the redundancy argument turns out (KTD12, KTD24).
...
- **Test scenarios:**
  1. A Windows `button` whose `LegacyIAccessible` state carries `STATE_SYSTEM_PRESSED` emits `pressed`. **Invert-verifiable.**
  2. A Windows `Button` advertising `ToggleAvailable` still resolves to `switch` with `checked` — the guard that `button_role` was not touched.
  3. macOS `cmd+shift+ctrl+q` is blocked. **Invert-verifiable — it is delivered today.**
  4. macOS `cmd+w` is still delivered — the guard that superset matching did not over-block. **And every one of the five `BLOCKED` entries is still refused, asserted entry by entry**, which is the monotonicity guarantee KTD24 requires: the test enumerates the list rather than spot-checking it, so no future edit can silently unblock one.
  5. The role/state conformance suite passes on both adapters with the vocabulary unchanged.
- **Verification:** scenarios 1 and 3 fail before and pass after; CI's macOS lane carries 3 and 4.
...
- **Landing target:** gate PR.
- **Goal:** a runner exists, its trigger policy matches what the workflow declares, and the accumulated queue is cleared — once the owner authorizes it (KTD18a, KTD18b).
- **Requirements:** R20.
- **Dependencies:** U1, and **an explicit owner authorization for registration** (KTD18a). This is the plan's one hard external dependency: without it U10 and U11 do not run, and R20, R21 and R22 are dispositioned rather than met.
...
- **Approach:**
  0. **Stop at the authorization boundary before anything else.** Registration is the owner's reserved decision (KTD18a), declined at §2.12 for a reason that still holds. The implementer prepares the runbook steps and confirms the repository's fork-PR approval setting is available to enable — and then **asks, and waits**. No token is generated and no runner is registered without an explicit go-ahead. If the answer is no, this unit and U11 stop here and U1 records Branch B; everything else in the gate proceeds.
  ...
  4. **Re-ratify the trigger policy against what is declared:** `workflow_dispatch` plus a paths-scoped `push` on `feat/windows-adapter`, never `pull_request`. **The fork-PR approval policy is set** — "Require approval for all outside collaborators" or stricter — because the runner's labels are reachable from every `pull_request`-triggered workflow in the repository, not only from `windows-e2e.yml` (KTD18b).
...
- **Test scenarios:**
  ...
  5. The repository's Actions setting requires approval for outside collaborators, asserted by reading it back after registration rather than from the runbook's instruction. **This is the guard the security review added** — without it a fork PR editing any of the five `pull_request`-triggered workflows could claim the runner's desktop.
  6. The runner is configured to clean its workspace between jobs, asserted by observing a second run start from a clean checkout rather than by reading the configuration. One run's build artifacts leaking into the next run's observations would make every e2e result suspect.
- **Verification:** one green `workflow_dispatch` run; the queue is empty; fork-PR approval reads back as enabled; a second run starts from a clean workspace; the runbook's owed section is filled.
...
- **Dependencies:** U10 — including its owner-authorization gate (KTD18a), without which this unit does not run — and every normalization PR that changes behaviour the harness asserts (U3, U5, U6, U7, U8).
- **Files:** `tests/e2e-windows/scenarios/Interaction.ps1`, `tests/fixture-app-windows/FixtureOcclusion.cs`, `tests/e2e-windows/LibEnvelope.psm1`, `tests/e2e-windows/scenarios/Acceptance.ps1`, `scripts/run-windows-e2e-ci.ps1`.
- **Approach:**
  1. **Re-baseline `headed-double-click` and `interaction-scroll-to-visibility`.** Both fail identically when this branch's diff is stashed, so neither is introduced here. The dogfood independently observed the occlusion half live: a tray-click preflight's hit-tests were occluded by the driving console's own window. The repair is a harness stage whose occluder is the harness's own topmost fixture rather than the operator's console, so the leg measures occlusion by something it controls. **It is mostly reuse, not new infrastructure:** `tests/fixture-app-windows/FixtureOcclusion.cs` already ships `open-occluder`/`close-occluder`, `LibEnvelope.psm1` ships `Assert-EnvelopeCheckOccluder`, and `scenarios/Acceptance.ps1` already exercises the present-then-closed pairing. The two legs adopt that pattern.
...
- **Test scenarios:**
  ...
  2. With the occluder stage skipped, the same leg's click succeeds cleanly and reports no occlusion failure — the guard that the stage's presence, not the leg itself, produced scenario 1's signal. (An earlier draft asked the leg to detect occlusion with no occluder present, which is unsatisfiable.)
...
| R2 | U1 scenario 6 — **reviewer-verified, the plan's one documented exception to the mechanical-guard rule**, with the reason stated | U1 |
...
| R36 | U7 scenarios 7-8 | U7 |
| R13 | U6 scenarios 1-3, 5 | U6 |
| R14 | U6 scenarios 6-7 | U6 |
| R15 | U6 scenarios 8-9 | U6 |
| R16 | U1 scenario 7 — reviewer-verified alongside R2; the contract sentence and both fidelities are in `docs/phases.md`, and A14-8's no-propagation ban is restated as binding on both adapters | U1 |
| R17 | U8 scenarios 1-2, 5 | U8 |
| R18 | U8 scenarios 3-4 | U8 |
...
| R20 | U10 scenarios 1-6 — including the fork-PR approval read-back and the clean-workspace observation. **Branch-conditional (KTD18a):** under Branch B this requirement is dispositioned in `docs/phases.md` rather than met, and U1 records it | U10 |
| R21 | U11 scenario 5 — both tiers green in one run. **Branch-conditional (KTD18a)**, as R20 | U11 |
| R22 | U11 scenarios 1-2 — the leg passes, and the occluder-skipped run shows the stage produced the signal. **Branch-conditional (KTD18a)**, as R20 | U11 |
...
| R34 | U1 scenario 9 — the plan's table and `docs/phases.md` are asserted against each other | U1 |
| R35 | U1 scenario 8 — an undisposed row naming 2.15 fails the ledger check at the gate | U1 |
...
- `windows-e2e.yml` green in both tiers on the registered runner — **under Branch A only**. Under Branch B (the owner maintains §2.12's refusal) this gate is not green because it cannot run, and U1 records R20, R21 and R22 as dispositioned instead (KTD18a).
...
- Every requirement R1-R36 is met and mapped to a test above, **except R2 and R16**, which are documentation-completeness requirements verified by the reviewer at close, **and R20, R21 and R22, which are conditional on an owner authorization this plan does not assume** (KTD18a) — under Branch B they are dispositioned in `docs/phases.md` with §2.12's reasoning restated, which is what closes them. That exception is stated here rather than papered over with a keyword grep that would false-pass; see U1 scenario 6.
...
- **Branch A:** the self-hosted runner is registered on the owner's authorization, its queue flushed, its policy re-ratified, fork-PR approval enabled, and the full live gate is green in both tiers on it. **Branch B:** the authorization was withheld, and R20/R21/R22 are written into `docs/phases.md` as blocked on the owner's reserved decision, with the live-gate exit criterion carried to §2.16 or ratified. One of the two holds; neither is left open.
...
| U7 one `--app` predicate, `renderer` | 2.15.4 | 240 |
| U8a `pressed` (Windows) | 2.15.5a | 90 |
| U8b superset blocked combos (macOS) | 2.15.5b | 70 |
| U9 rig measurements and ratifications | gate | 320 (probe + ledger + doc) |
| U10 runner registration | gate | 110 (runbook + doc) |
...
| The superset blocked-combo change blocks a combo a macOS user relies on | low | medium | the entry list is not edited at all, so the change is monotonic - nothing blocked today can become unblocked (KTD24); U8 scenario 4 enumerates all five entries and pins that `cmd+w` still delivers; it ships with a release note and is named in the plan as the one GA behaviour change |
...
| The owner maintains §2.12's refusal, so the branch never gets a live e2e run | **medium** | high | KTD18a pre-commits both branches, so this is a disposition rather than a surprise; every unit except U10 and U11 lands regardless, and R20/R21/R22 carry to §2.16 or are ratified with §2.12's own reasoning |
...
None. All twenty "Settle" and "Decide here" bullets in §2.15's scope are answered as a numbered Key Technical Decision above, with its evidence, its rejected alternatives and its landing target. Three of those answers are the user's to reverse and are flagged rather than buried — KTD12's macOS GA behaviour change, KTD16's four rig ratifications, and KTD17's ratification that macOS keeps the trait default for the three shell-surface kinds.
...
**`docs/phases.md` §2.15 claims measured false** (each becomes a U1 correction citing the evidence in parentheses). The identifiers are planning's own working labels. **C2 is absent because it was folded into R2**, and **C19 is listed but withdrawn — it is not one of the twenty-one** — the exit criteria failing the DoD's enumerate-do-not-gesture rule is a requirement of this plan rather than a claim to correct:
...
- C19 — **withdrawn, and recorded here because the plan was wrong rather than the document.** An earlier draft observed that `windows-e2e.yml` has no `pull_request` trigger and concluded §2.15's instruction to set the fork-PR approval policy was moot. Runner labels are repository-wide, five other workflows trigger on `pull_request`, and `docs/phases.md:1277` had already recorded the owner declining registration on exactly that exposure. §2.15's instruction is correct as written and needs no correction. C19 corrects nothing and is not counted among the twenty-one.
