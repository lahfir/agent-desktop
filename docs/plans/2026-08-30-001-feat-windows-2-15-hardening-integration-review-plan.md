...
| R20 | U10 scenarios 1-6 — including the fork-PR approval read-back and the clean-workspace observation | U10 |
...
- Every requirement R1-R36 is met and mapped to a test above, **except R2 and R16**, which are documentation-completeness requirements verified by the reviewer at close. That exception is stated here rather than papered over with a keyword grep that would false-pass; see U1 scenario 6.
...
- C19 — `windows-e2e.yml` has no `pull_request` trigger. **This does not make the fork-PR approval instruction moot**, which an earlier draft of this plan concluded and was wrong about: runner labels are repository-wide and five other workflows trigger on `pull_request`, so the policy is set at registration (KTD18b). What C19 corrects is only §2.15's implication that the policy gates *this* workflow's fork PRs — it gates every workflow's.
