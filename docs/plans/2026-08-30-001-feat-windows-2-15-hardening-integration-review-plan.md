...
- **Approach:**
  ...
  10. **KTD10's `offscreen` contract.** One sentence that is true of both adapters — *the element is not presently rendered within the visible region of its container* — with each adapter's source named beside it (macOS geometric against the window rectangle because `AXOffscreen` is not published; Windows the provider's per-element value, which also sees scroll-viewport clipping, per A18-1), and A14-8's no-propagation ban restated as binding on both.
