...
production-readiness work — nothing in core is rebuilt.
...
│   │   ├── screenshot.rs               # ScreenshotBackend legacy capture
...
pub trait PlatformAdapter: Send + Sync {
    ...
    fn execute_action(&self, handle: &NativeHandle, request: ActionRequest)
...
- `crates/macos/src/adapter.rs`: `MacOSAdapter::get_tree()` and `MacOSAdapter::permission_report()` implemented; all other methods stub
...
- `screenshot.rs` (ScreenshotBackend legacy capture for window; full screen)
...
| P4: Production Readiness | 8 weeks | Persistent daemon, session isolation, brew/winget/snap packages, performance benchmarks |
