...
**Scope:**
- `launch_app` with `LaunchOptions` (`CreateProcessW` args/env/cwd — Windows honours a launch `cwd` where macOS Launch Services rejects it — attach-vs-fail policy via `attach_if_running`). `CreateProcessW` is chosen over `ShellExecuteEx` because it is manifest-ready under the already-enabled `Win32_System_Threading` feature and returns a `PROCESS_INFORMATION` handle that is a strong single identity primitive, so Windows needs less "launched" verification ceremony than macOS's four-signal `NSWorkspace` dance. UWP launch by AUMID (`IApplicationActivationManager`) is out of scope: like `IVirtualDesktopManager` (line 1146), the pinned crates do not generate the interface, so it stays deferred unless a stable binding path is validated
