...
**Inherited defect, found by §2.15's full-branch review and not fixed there.** `dismiss-all --app <name>` on Windows destroys notifications belonging to every other application. `dismiss_all_impl` (`crates/windows/src/notifications/actions.rs`) narrows its captured entry set to the filter and then invokes the Action-Center-wide `ClearAllButton`, because no per-app clear control exists. The post-invoke re-read is filtered by the same predicate, so the envelope reports a clean scoped success while unrelated notifications are gone with no record. macOS does not have this: its dismiss-all walks the filtered entries and dismisses each through its own control. No test covers a non-`None` filter, which is why it survived.

**The shape of the fix is known and the first attempt at it failed, so it is written down rather than guessed at again.** When a filter is present, dismiss each captured entry through its own per-entry control instead of the global clear-all. The attempt made during §2.15 broke six tests in the notification suite: it discarded per-entry errors, dismissed entries by index while iterating so the remaining elements went stale, and disturbed the shared toast staging helper. A working fix must dismiss against stable element identity rather than a shifting index, must surface per-entry failures through the existing survivor comparison rather than swallowing them, and must leave the staging scaffolding alone.
