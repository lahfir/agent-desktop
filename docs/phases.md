...
- **The promotion's release note must carry one behavioural change core already made.** `AppError::code` now answers `PERM_DENIED` for an io error whose kind is `PermissionDenied`; on `main` today every io error still answers `INTERNAL`. The new code is the honest one — `INTERNAL` says the tool broke and carries no action — but a consumer branching on `INTERNAL` to catch a permission failure stops matching the moment Windows lands. `main` has never shipped the new mapping, so there is nothing to deprecate and no alias to add; what is required is that the `feat!` promotion says so in its release note rather than letting a caller discover it. Introduced by `07ccbddc`.
...
**Goal:** Prove the assembled `feat/windows-adapter` branch is production-grade as a whole. The promotion to `main` moves to §2.16, which §2.17 follows.
...
- **`agent-desktop-windows` is not built for non-Windows targets, and that is the decision rather than an oversight.** Nothing links the crate on a Linux target: the binary and the FFI crate declare their platform crates under `[target.'cfg(target_os = "windows")'.dependencies]`, so a Linux build never compiles `agent-desktop-windows`. What is gated in its place is core's platform neutrality — `agent-desktop-core` cross-compiles to `x86_64-unknown-linux-gnu` and `x86_64-pc-windows-msvc`, and `cargo tree -p agent-desktop-core` proves core carries no platform crate. Those are the invariants that matter, and both hold. Repairing the non-Windows build instead needs `#[cfg(not(target_os = "windows"))]` stubs across roughly twenty files that no lane executes — the exact shape `docs/solutions/best-practices/never-ship-platform-code-that-ci-cannot-execute.md` records the repository paying for once. This gate's scope offered a lane **or** a recorded exclusion with its reason; this is that exclusion. **Evidence:** `docs/solutions/best-practices/never-ship-platform-code-that-ci-cannot-execute.md`
...
- Hand §2.16 a settled contract and a proven branch. **This gate does not merge to `main`** — §2.16 carries the promotion, which its own Sequencing and Exit criteria state
...
**Sequencing:** it follows §2.15 because it implements against the response-shape decision §2.15 takes; §2.17 follows it. **It delivers the promotion checklist, not the promotion.** The merge of `feat/windows-adapter` into `main` is a separate, later, release-noted `feat!` that runs that checklist once this sub-phase has merged into the integration branch — a sub-phase PR into `main` is forbidden, so the phase cannot close inside this one.

**§2.15's full-branch review findings are closed in `crates/windows`, and this sub-phase inherits none of them.** §2.15 reviewed the assembled `feat/windows-adapter` branch — 1,161 files, ~193k insertions — one reviewer per subsystem, and closed each finding with a named invert-verified test. A timed-out clipboard read can no longer leave a worker holding the clipboard open: `crates/windows/src/input/clipboard_worker_state.rs` holds an `OUTSTANDING` counter and `ensure_no_outstanding_worker()` guards the four call sites in `clipboard.rs`. `snapshot --surface menu` and the `surface-appeared` wait resolve one predicate, because `locate_menu` (`crates/windows/src/system/menu_state_locate.rs`) composes the tool-window source and `locate_chromium`. Two fault-versus-absence collapses are separated: `first_native_hwnd` returns `Result<Option<isize>, BudgetExpired>`, and `list_displays_live` captures the `EnumDisplayMonitors` return and carries `info_read_failed`. `resolve_actions` (`crates/windows/src/tree/actions.rs`) gates `set-value` on `RangeValueIsReadOnly`; `execute_chain` carries `exhaustion_disposition(&steps)` on expiry, so a rung's budget expiry no longer discards the steps before it; `resolved_from_scan` (`crates/windows/src/input/keyboard_map.rs`) extracts `VkKeyScanW`'s high byte, so key synthesis honours the layout's shift requirement; and `list_surfaces_for_process` builds its observations without per-window propagation, so one unresponsive window no longer aborts an inventory read. The five e2e legs that could not fail now carry `Add-Fail` gating and a pixels-produced check. `get --property text` is role-conditional through core's `role_text::value_is_the_readable_text`.
...
**`screenshot_window_frame` captures one named window, not the foreground.** Its only consumer is the visual-debug artifact, and only on one of its two paths: `src/visual_debug/capture.rs:176` calls it for `click --debug --screenshot <path>.html`, while the `snapshot` path uses the ordinary `screenshot` the Windows adapter already implements. So visual debug is half-available on Windows right now — measured on the 0.9.0 binary against a live Settings window, `snapshot --debug --screenshot` writes a 16 KB artifact while `click --debug --screenshot` refuses `PLATFORM_NOT_SUPPORTED` ("screenshot_window_frame is not supported on this platform") with `not_delivered` / `retry: safe`, so the click is correctly never delivered. That is worse than either whole state, because the flag is global and its failure is per-command. The macOS implementation resolves the window through `resolve_window_strict`, re-verifies bounds before and after the capture, and fails closed if the window moved between them; a Windows implementation that captures whatever is frontmost would produce a debug artifact that confidently shows the wrong application.

**Two adjacent gaps this sub-phase also owns, both surfaced by the 0.9.0 sync and neither introduced by it.**

*A `menuitem` that expands is verified against evidence nothing guarantees.* `is_expandable_role` (`crates/core/src/roles.rs`) covers `combobox`, `menubutton`, `treeitem` and `disclosure` — not `menuitem`. So a `menuitem` advertising `Expand` reports `states_complete` trivially true, while `push_expand_collapse_state` (`crates/windows/src/tree/states.rs`) pushes the `expanded` token for *any* role. An unreadable `ExpandCollapseState` on such an element is therefore indistinguishable from a collapsed one, which is exactly the false `ACTION_FAILED` the adapter's completeness predicate now prevents one role over. Read from the code rather than measured: the live probe that found such an element on the Settings menu bar refused earlier, at the `visible` check, because its bounds were zero-sized. The predicate is core-shared, so macOS carries the same shape. What closes it: either `menuitem` joins the expandable roles, or the token push is role-gated the way the toggle push already is — and whichever is chosen, both adapters move together.

*The value-write chain still advances after an unverified delivery.* `record_step_outcome` (`crates/windows/src/actions/chain.rs`) advances to the next rung when the outcome does not terminate the chain **or** when `continue_after_unverified_delivery` is set and the outcome is `DeliveredUnverified` — and `VALUE_WRITE_CHAIN` sets it. So a rung whose write landed but whose readback failed is followed by a second mechanism against a control that has already been mutated. The error path is already correct: any `Err` aborts the chain and never falls through. 0.9.0 tightened precisely this progression on the macOS side (`select_menu.rs`), so the two adapters now disagree about a rule whose whole purpose is to prevent double delivery. What closes it: the same rule here — advance only on proven non-delivery — with the `continue_after_unverified_delivery` flag either removed or justified per chain against a named double-delivery risk.
