...
**Exit criteria:** every item in the Cross-cutting sub-phase DoD holds for the whole branch; both cross-platform contract decisions above are settled in this document rather than left to the next platform to inherit; `main` gains Windows support in one commit.

**Est. PR size:** ~500 LOC on top of a large verification effort. The error-payload promotion is the one code change this gate is certain to carry — core constructors plus both adapters and a core test — and either contract decision above may add a normalization change on one adapter. Everything else is review, live e2e, perf baseline, audits and the merge.
