...
- `scroll_into_view` via `ScrollItemPattern` — the only call in this sub-phase that mutates the target rather than reading it; its HRESULT handling is completed by 2.7's mutation-path delivery classifier, which routes this call site through itself when it lands
