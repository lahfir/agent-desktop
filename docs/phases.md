...
| `list-tray-items` / `click-tray-item` / `open-tray-menu` | Not shipped: measurement moved the notification area under this table's own generic rule. Its items are `Button` elements with stable machine-local `AutomationId`s and `InvokePattern` on positive-area bounds, so they carry refs and the stated preference applies — `snapshot --surface system-tray` (three promoted items) and `snapshot --surface system-tray-overflow` (five) expose them and `click @ref` invokes them (A26-5, A26-7) | Reached through `Shell_TrayWnd` plus the overflow flyout (`TopLevelWindowForOverflowXamlIsland` on Win11 22H2+; `NotifyIconOverflowWindow` before that — present and readable first-party on this build, C-5, A26-6); macOS maps to menu bar extras. Linux maps to StatusNotifier in Phase 3 |
