...
mod imp {
    ...
    pub(crate) fn press_to_expand(
        ...
        press_toggle_disclosure(el, true)
    }
    ...
    pub(crate) fn press_to_collapse(
        ...
        press_toggle_disclosure(el, false)
    }
    ...
    fn press_toggle_disclosure(el: &AXElement, want_expanded: bool) -> Result<bool, AdapterError> {
        ...
        if ax_helpers::has_ax_action(el, action) {
            ...
            if disclosure_settled(el, want_expanded) {
                return Ok(true);
            }
        }
        ...
        if ax_helpers::has_ax_action(el, "AXPress")
            ...
            && disclosure_settled(el, want_expanded)
        {
            return Ok(true);
        }
        Ok(false)
