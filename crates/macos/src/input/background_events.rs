use agent_desktop_core::{AdapterError, MouseButton, MouseEvent, MouseEventKind};
use core_graphics::event::{CGEvent, CGEventFlags, CGEventType, EventField, ScrollEventUnit};
use core_graphics::geometry::CGPoint;
use std::time::Duration;

use crate::input::mouse::{
    create_event_with_source, down_type, event_source, standalone_state_error, to_cg_button,
    up_type, wheel_lines_to_i32,
};
use crate::input::mouse_scroll::scroll_chunks;
...
/// Gap between wheel chunks, matching the headed wheel path.
const WHEEL_CHUNK_GAP: Duration = Duration::from_millis(5);
...
pub(crate) enum Phase {
    ...
    Down,
    Up,
    /// One line-unit scroll-wheel event; positive `dy` is up, positive `dx`
    /// is left.
    Wheel {
        dy: i32,
        dx: i32,
...
/// `1..=count`. A wheel becomes the same optional preceding move, then the
/// line deltas split into the headed path's per-event chunks; the primer
...
pub(crate) fn plan(
    ...
    let count = match event.kind {
        ...
        MouseEventKind::Wheel { delta_x, delta_y } => {
            let chunks =
                scroll_chunks((wheel_lines_to_i32(delta_y)?, wheel_lines_to_i32(delta_x)?))?;
...
/// Builds, without posting, the `CGEvent` for `planned`. Fields 91/92 are
/// always set, plus the click state on button events. The `route` layer adds
/// the target pid (40) and Warp's window/click-group fields (51/58), and on
/// button events the button number, the touch subtype, and pressure (1.0
/// while a button is down, as Warp does).
...
pub(crate) fn build(
    ...
    let event_type = match planned.phase {
        ...
        Phase::Wheel { dy, dx } => return build_wheel(source, planned, routing, (dy, dx)),
    };
    ...
    set_window_fields(&event, routing);
    if !routing.route {
        return Ok(event);
    }
...
/// Line units, like the headed `mouse-wheel`: AppKit and Chromium both turn a
/// non-continuous line delta into their own per-line scroll distance, so no
/// scroll phases or momentum are synthesized.
///
/// CoreGraphics silently drops fields 91/92 on scroll events (they are
/// mouse-event fields), so without `route` a wheel is identified only by its
/// pid and global location; `route` adds the pid, window number (51), and the
...
fn build_wheel(
    ...
    event.set_location(planned.global);
    event.set_flags(routing.flags);
    set_window_fields(&event, routing);
    Ok(event)
}

/// Fields 91/92 always name the target window; `route` adds the target pid
/// and Warp's window and click-group fields.
fn set_window_fields(event: &CGEvent, routing: &EventRouting) {
    event.set_integer_value_field(
