...
fn search_tree(
    ...
    let text_match = args.text.as_deref().is_none_or(|t| {
        ...
        let in_name = node.name.as_deref().is_some_and(|n| n.to_lowercase().contains(&t_lower));
        let in_value = node.value.as_deref().is_some_and(|v| v.to_lowercase().contains(&t_lower));
        let in_desc = node.description.as_deref().is_some_and(|d| d.to_lowercase().contains(&t_lower));
        in_name || in_value || in_desc
    });

    if role_match && name_match && value_match && text_match {
        let interactive = node.ref_id.is_some();
        let display_name = node.name.as_deref()
