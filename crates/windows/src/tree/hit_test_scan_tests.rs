/// The file the `get_pattern` ban is lifted for, named once so the allowlist
/// is a single identifier rather than a string repeated at each use.
...
/// everywhere but `actions/scroll_into_view.rs`: `ScrollItemPattern` exposes
/// no property-shaped equivalent, so its one invocation is the only way to ask
/// a provider to bring an item into view. The allowlist is one file matched by
/// name, so a second call site anywhere - `tree/` included - fails here.
#[test]
fn the_occlusion_gate_issues_no_banned_call() {
    ...
    for (name, source) in scanned() {
        for (number, line) in code_lines(source) {
            ...
            assert!(
                name == PATTERN_ALLOWLIST || !line.contains(gated),
                "{name}:{number} instantiates a pattern; only {PATTERN_ALLOWLIST} may: {line}"
            );
        }
    }
}
...
/// If `ScrollIntoView` stops needing a pattern handle, this fails and the
...
fn the_pattern_allowlist_covers_a_call_site_that_is_still_there() {
    let (name, source) = GATE_SUPPORT[0];
    assert_eq!(name, PATTERN_ALLOWLIST);
    assert!(
        code_lines(source).any(|(_, line)| line.contains(concat!("get_", "pattern"))),
        "{PATTERN_ALLOWLIST} no longer instantiates a pattern, so its allowlist entry is dead"
    );
}
