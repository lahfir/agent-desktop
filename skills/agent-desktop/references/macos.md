...
macOS keeps one click-through AppKit child alive for the enabled session, with separate borderless windows for the cursor, the card, the ripple, and the element outline. Enable applies the session style and shows “Hey, let's play with this computer!”, and eases away only the card. Later actions start from the cursor's previous destination and follow the shared human path. The child receives the move before the action dispatches and the click effect after it confirms, so the two overlap. The cursor is a filled dart with a contrasting rim; it never rotates or resizes. A click draws a contact glow and two expanding rings in the accent colour, then outlines the element with a rounded accent border that Core Animation fades over 2.4 seconds without a block on the render loop. After 6 idle seconds the cursor and card fade out; the next command restores them. `cursor-overlay disable` removes everything and stops the child. Headed actions temporarily hide it while the real cursor is in use. The overlay never activates, moves the OS pointer, or changes command delivery, follows Reduce Motion, and fails soft if its child cannot start.
