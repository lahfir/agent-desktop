...
- `press --background --window-id w-N` and `type --background` (a ref, or `--window-id`) post key events to that window's process after target-only records make it the key window inside its own process (activation record plus yabai's make-key pair), with an `SLSEventAuthenticationMessage` attached for Chromium. The app is not activated on purpose, but keeping it inactive and leaving the user's keyboard focus alone is best effort: check `focus_change` and `focus_guard` in the result. No key is mapped to a menu or AX action. Text is Unicode per character; results are `delivered_unverified`, confirmed by `snapshot`. `AGENT_DESKTOP_BG_LAYERS` also accepts `auth` and `keywindow`; see `docs/solutions/best-practices/background-keyboard-delivery-2026-09-24.md`.
