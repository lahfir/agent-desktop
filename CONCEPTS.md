...
An optional presentation-only cursor driven by verified action destinations. Its configuration — label, colours, size, and click effects — is enabled or disabled once in a session manifest with `cursor-overlay`; action and batch commands inherit it without cursor flags. Motion models a human hand: a ballistic primary submovement, a small overshoot, a corrective submovement, a bowed path, and light tremor. The cursor never rotates or resizes. It travels to the destination before the action dispatches, and the platform renderer acknowledges arrival, so the visible cursor and the real effect stay in step. A click then plays a ripple at the destination and flashes an outline around the clicked element. The wait is bounded; a slow or missing renderer never blocks the action. The overlay fades out after an idle period and returns on the next command. Headed mode suppresses it because the real pointer already represents the action. It never moves or replaces the OS pointer and never changes Interaction Policy, action delivery, or response semantics. Shared configuration, motion, effect timing, and the arrival contract stay platform-neutral in `agent-desktop-core`; macOS supplies the native renderer while other platforms inherit the adapter's default no-op.
